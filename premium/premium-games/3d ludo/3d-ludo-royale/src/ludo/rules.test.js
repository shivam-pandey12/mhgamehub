...
import { FINAL_STEP, LAST_TRACK_STEP } from './constants.js';
import { getCellForSteps } from './layout.js';
...
describe('3D Ludo Royale rules engine', () => {
  ...
  it('keeps the same player after a no-move six', () => {
    ...
    for (let index = 0; index < 4; index += 1) {
      game.setTokenForTest('red', index, { state: 'finished', steps: FINAL_STEP });
    }
  ...
  it('enters home lane after completing the common route', () => {
    const game = new LudoGame(2);
    game.setTokenForTest('red', 0, { state: 'track', steps: LAST_TRACK_STEP });

    game.rollDice(1);
    const result = game.moveToken('red-0');

    assert.equal(game.state.tokens.red[0].state, 'home-lane');
    assert.equal(game.state.tokens.red[0].steps, LAST_TRACK_STEP + 1);
    assert.equal(result.path[0].kind, 'home-lane');
    assert.deepEqual(result.path[0].cell, { x: 7, y: 13 });
    assert.deepEqual(getCellForSteps('red', LAST_TRACK_STEP).cell, { x: 7, y: 14 });
  });

  it('enters each color home lane from the center approach without side detours', () => {
    const expectations = {
      red: [{ x: 7, y: 14 }, { x: 7, y: 13 }],
      blue: [{ x: 7, y: 0 }, { x: 7, y: 1 }],
      green: [{ x: 0, y: 7 }, { x: 1, y: 7 }],
      yellow: [{ x: 14, y: 7 }, { x: 13, y: 7 }]
    };

    for (const [playerId, [approach, entry]] of Object.entries(expectations)) {
      assert.deepEqual(getCellForSteps(playerId, LAST_TRACK_STEP).cell, approach);
      assert.deepEqual(getCellForSteps(playerId, LAST_TRACK_STEP + 1).cell, entry);
      assert.equal(getCellForSteps(playerId, LAST_TRACK_STEP + 1).kind, 'home-lane');
    }
  });
  ...
  it('detects winner when all four tokens reach home', () => {
    ...
    game.setTokenForTest('red', 3, { state: 'home-lane', steps: FINAL_STEP - 1 });
