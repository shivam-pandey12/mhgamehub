(function(){let e=document.createElement(`link`).relList;if(e&&e.supports&&e.supports(`modulepreload`))return;for(let e of document.querySelectorAll(`link[rel="modulepreload"]`))n(e);new MutationObserver(e=>{for(let t of e)if(t.type===`childList`)for(let e of t.addedNodes)e.tagName===`LINK`&&e.rel===`modulepreload`&&n(e)}).observe(document,{childList:!0,subtree:!0});function t(e){let t={};return e.integrity&&(t.integrity=e.integrity),e.referrerPolicy&&(t.referrerPolicy=e.referrerPolicy),e.crossOrigin===`use-credentials`?t.credentials=`include`:e.crossOrigin===`anonymous`?t.credentials=`omit`:t.credentials=`same-origin`,t}function n(e){if(e.ep)return;e.ep=!0;let n=t(e);fetch(e.href,n)}})();function e({label:e,action:t=``,variant:n=`primary`,size:r=`md`,disabled:i=!1,attrs:a={}}){let o=t?`data-action="${t}"`:``,s=Object.entries(a).filter(([,e])=>e!=null&&e!==!1).map(([e,t])=>`${e}="${String(t)}"`).join(` `);return`
  ...
  `}function Re(t){if(!t.room?.draftState)return K({kicker:`Draft`,title:`Draft not ready`,content:`
                ...
                ${e.history.map(e=>`
                      <article class="history-item">
                        <div>
                          ...
                          <p>Room ${e.roomCode} &middot; ${pe(e.playedAt)}</p>
                        </div>
                        <div>
                          <p>${me(e.settings?.matchMode??`two-batsmen`)} &middot; ${me(e.settings?.bowlingMode??`over-locked`)}</p>
  ...
  `}function Be(){let e=Pe(),t={installedVersion:e.installedVersion,whatsNewVersion:e.whatsNewVersion},n=!1;t.installedVersion?t.installedVersion!==W.version&&(t.installedVersion=W.version,t.whatsNewVersion=W.version,n=!0):(t.installedVersion=W.version,n=!0);let r=Date.parse(W.releasedAt);if(Number.isNaN(r))return n&&Fe(t),!1;let i=W.whatsNewVisibleDays*24*60*60*1e3,a=Date.now(),o=a>=r&&a<r+i,s=t.whatsNewVersion===W.version&&o;return!o&&t.whatsNewVersion===W.version&&(t.whatsNewVersion=null,n=!0),n&&Fe(t),s}function Ve(t){let n=t.lastSavedMatch,r=Be(),i=W.whatsNewUrl.trim();return`
        ...
        <div class="field-grid">
          ...
          <label class="field">
            ...
            <select data-field="players-per-team">
              ${be.map(e=>`<option value="${e}" ${t.ui.playersPerTeam===e?`selected`:``}>${e} vs ${e}</option>`).join(``)}
            </select>
          </label>
        </div>
  ...
  `}function He({title:e,teamId:t,type:n,order:r,playersById:i,movableIds:a=null,locked:o=!1,helper:s=``}){return`
  ...
  `}function Ue(t){if(!t.room?.teams.alpha||!t.room.teams.beta)return``;let n=Object.fromEntries(t.room.players.map(e=>[e.id,e])),r=t.room.lineups?.alpha??{battingOrder:t.room.teams.alpha.battingOrder,bowlingOrder:t.room.teams.alpha.bowlingOrder},i=t.room.lineups?.beta??{battingOrder:t.room.teams.beta.battingOrder,bowlingOrder:t.room.teams.beta.bowlingOrder},a=t.session.localPlayerId,o=t.room.settings.controlMode===_e.HOTSEAT,s=o||t.room.captains.alpha===a,c=o||t.room.captains.beta===a,l=t.room.hostId===a,u=n[t.room.captains.alpha??``]?.name??`Alpha captain`,d=n[t.room.captains.beta??``]?.name??`Beta captain`;return`
    <section class="page-stack">
      ${K({kicker:`Ready`,title:`Lock lineups and start the match`,className:`card--hero`,content:`
        ...
        `})}
      <div class="grid-two">
        ${K({kicker:`Lineups`,title:t.room.teams.alpha.name,content:`
            ${He({title:`Batting Order`,teamId:`alpha`,type:`batting`,order:r.battingOrder,playersById:n,locked:!s,helper:s?`Alpha captain sets the order for incoming batters.`:`${u} controls this batting lineup.`})}
            ${He({title:`Bowling Rotation`,teamId:`alpha`,type:`bowling`,order:r.bowlingOrder,playersById:n,locked:!s,helper:s?`Set the starting bowling order. Free-change mode can still adjust this in-match.`:`${u} controls this bowling lineup.`})}
          `})}
        ${K({kicker:`Lineups`,title:t.room.teams.beta.name,content:`
            ${He({title:`Batting Order`,teamId:`beta`,type:`batting`,order:i.battingOrder,playersById:n,locked:!c,helper:c?`Beta captain sets the order for incoming batters.`:`${d} controls this batting lineup.`})}
            ${He({title:`Bowling Rotation`,teamId:`beta`,type:`bowling`,order:i.bowlingOrder,playersById:n,locked:!c,helper:c?`Set the starting bowling order for match setup.`:`${d} controls this bowling lineup.`})}
          `})}
      </div>
    </section>
  `}function We({player:e,badges:t,rowClass:n=``,actions:r=``,layout:i=`default`}){let a=e.status===`unready`?`not ready`:e.status;return`
  ...
  `}function Ge({players:t,captains:n,teams:r={alpha:null,beta:null},hostId:i=``,localPlayerId:a=null,canRemovePlayers:o=!1}){return`
    <ul class="player-list">
      ${t.map(t=>{let s=r.alpha?.playerIds.includes(t.id)?`Alpha`:r.beta?.playerIds.includes(t.id)?`Beta`:``;return We({player:t,badges:[t.id===i?`Host`:``,t.id===n.alpha?`Alpha Captain`:``,t.id===n.beta?`Beta Captain`:``,s,t.isLocal?`You`:``,t.status===`ready`?`Ready`:`Waiting`].filter(Boolean),rowClass:[t.id===i?`is-host`:``,t.id===n.alpha||t.id===n.beta?`is-captain`:``,t.status===`ready`?`is-ready`:``,t.id===a?`is-local`:``].filter(Boolean).join(` `),actions:o&&t.id!==i?e({label:`Remove`,action:`remove-player`,variant:`danger`,size:`sm`,attrs:{"data-player":t.id}}):``})}).join(``)}
    </ul>
  `}function Ke({room:t,localPlayerId:n}){let r=t.teamSelections??{alpha:[],beta:[]},i=Object.fromEntries(t.players.map(e=>[e.id,e])),a=new Set([...r.alpha,...r.beta]),o=t.playerIds.filter(e=>!a.has(e)),s=t.hostId===n,c=t.teams.alpha?.name??t.teamNames?.alpha??`Alpha XI`,l=t.teams.beta?.name??t.teamNames?.beta??`Beta XI`;function u(i,a){if(!(s||i===n))return``;let o=r.alpha.length>=t.settings.playersPerTeam,c=r.beta.length>=t.settings.playersPerTeam;return`
    ...
    `}function d(e,r,a,o){return`
      <section class="team-board__column ${r?`is-${r}`:`is-waiting`}">
        ...
        <ul class="player-list">
          ${e.length?e.map(e=>{let a=i[e];return a?We({player:a,badges:[a.id===t.hostId?`Host`:``,a.id===t.captains.alpha?`Alpha Captain`:``,a.id===t.captains.beta?`Beta Captain`:``,a.id===n?`You`:``].filter(Boolean),rowClass:[a.id===n?`is-local`:``,a.id===t.captains.alpha||a.id===t.captains.beta?`is-captain`:``,a.status===`ready`?`is-ready`:``].filter(Boolean).join(` `),actions:u(e,r),layout:`board`}):``}).join(``):`<li class="team-board__empty">No players here yet.</li>`}
        </ul>
      </section>
    `}return`
  ...
  `}function qe(t){if(!t.room)return``;let n=t.room.teamSelections??{alpha:[],beta:[]},r=(e,r)=>n[e].map(e=>t.room.players.find(t=>t.id===e)).filter(Boolean).map(e=>`<option value="${e.id}" ${e.id===r?`selected`:``}>${e.name}</option>`).join(``),i=t.room.hostId===t.session.localPlayerId,a=(t.room.players.find(e=>e.id===t.session.localPlayerId)??null)?.status===`ready`,o=t.room.players.filter(e=>e.status===`ready`).length,s=t.room.playerIds.length>=t.room.maxPlayers,c=s&&o===t.room.players.length,l=n.alpha.length===t.room.settings.playersPerTeam&&n.beta.length===t.room.settings.playersPerTeam,u=s&&c&&l&&t.room.captains.alpha&&t.room.captains.beta,d=t.room.draftState?.status===`complete`,f=t.room.source===`offline`,p=f||i,m=(t.room.teams.alpha?.name??t.room.teamNames?.alpha??`Alpha XI`).replace(/"/g,`&quot;`),h=(t.room.teams.beta?.name??t.room.teamNames?.beta??`Beta XI`).replace(/"/g,`&quot;`),g=r(`alpha`,t.room.captains.alpha),_=r(`beta`,t.room.captains.beta),y=!!g,te=!!_,ne=ee(),b=t.room.status===`live`||t.room.status===`completed`||!!t.match,x=i&&!b,C=t.ui.numberSetDraft??null,w={...t.room.settings,...C??{}},T=w.numberSetMode??`classic`,re=w.numberSetPreset??`classic`,ie=Array.isArray(t.room.settings.allowedNumbers)&&t.room.settings.allowedNumbers.length?t.room.settings.allowedNumbers:[1,2,3,4,5,6],E=S(w,t.room.settings),D=E.ok?E.settings.allowedNumbers:ie,O=E.ok?E.settings.numberSetLabel:`Pending edits`,k=t.ui.numberSetValidationError||(E.ok?``:E.error),A=String(w.customNumbersText??w.allowedNumbers?.join(`, `)??`1, 2, 3, 4, 5, 6`).replace(/"/g,`&quot;`);return`
    <section class="page-stack">
      ${K({kicker:`Lobby`,title:`Match Configuration`,content:`
          ...
          <section class="number-set-panel" aria-label="Number set settings">
            <div class="number-set-panel__head">
              ...
              <div class="number-set-preview">
                ...
                <strong>${O}</strong>
                <div class="number-set-list">${v(D)}</div>
              </div>
            </div>
            <div class="field-grid">
              <label class="field">
              ...
              <label class="field">
                ...
                <input data-field="custom-numbers" value="${A}" placeholder="1, 5, 7, 11, 20" ${!x||T!==`custom`?`disabled`:``} />
              </label>
              <label class="field">
                <span>Active Numbers</span>
                <input value="${t.room.settings.numberSetLabel??`Classic 1-6`}: ${ie.join(`, `)}" disabled />
              </label>
            </div>
            ...
            <div class="number-set-panel__actions">
              ${k?`<p class="validation-text">${k}</p>`:`<p class="muted">Active picker order is sorted ascending and supports up to 16 numbers.</p>`}
              ${e({label:`Apply Number Set`,action:`apply-number-set`,variant:`secondary`,disabled:!x||!!k})}
            </div>
          </section>
          <div class="field-grid">
            <label class="field">
        ...
        `})}
      <div class="grid-two">
        ${K({kicker:`Choose Sides`,title:`Alpha And Beta Areas`,className:`card--span-2`,content:Ke({room:t.room,localPlayerId:t.session.localPlayerId})})}
        ${K({kicker:`Room`,title:`Players`,content:Ge({players:t.room.players,captains:t.room.captains,teams:t.room.teams,hostId:t.room.hostId,localPlayerId:t.session.localPlayerId,canRemovePlayers:i&&!f})})}
  ...
  `}function Je(e,t={}){let n=Array.isArray(e)?e:[],r=n.map((e,t)=>({event:e,index:t})).slice(-12).reverse();return`
    <section class="ball-timeline">
      ...
      <div class="ball-timeline__list">
        ${r.length?r.map(({event:e,index:r})=>{let i=e.resultTone??e.resultType??`run`,a=e.resultLabel??(e.resultType===`wicket`?`Wicket!`:`+${e.runs}`),o=fe(e,n.slice(0,r),t.allowedNumbers),s=ue(t.match,n.slice(0,r+1),t.allowedNumbers),c=o||(s.label===`Neutral`?e.shotLabel??``:s.label);return`
  ...
  `}function Ye({title:e,side:t,selected:n,disabled:r,locked:i,busy:a=!1,note:o,statusLabel:s=``,allowedNumbers:c=y().allowedNumbers,waiting:l=!1}){let u=Array.isArray(c)&&c.length?c:y().allowedNumbers,d=u.length===6&&u.every((e,t)=>e===t+1),f=!r&&!i&&n!=null;return`
  ...
  `}var Ze=new Set;function q(e=`id`){return`${e}-${Math.random().toString(36).slice(2,8)}`}function Qe(){let e=``;do e=Array.from({length:6},()=>`ABCDEFGHJKLMNPQRSTUVWXYZ23456789`[Math.floor(Math.random()*32)]).join(``);while(Ze.has(e));return Ze.add(e),e}var $e=[{id:`batting`,label:`Batting Signals`,signals:[{id:`take-strike`,text:`I'll take strike`},{id:`you-take-strike`,text:`You take strike`},{id:`play-safe`,text:`Play safe`},{id:`go-aggressive`,text:`Go aggressive`}]},{id:`bowling`,label:`Bowling Signals`,signals:[{id:`i-bowl-next`,text:`I'll bowl next`},{id:`change-bowler`,text:`Change bowler`},{id:`try-random`,text:`Try random`},{id:`watch-pattern`,text:`Watch pattern`}]},{id:`general`,label:`General Signals`,signals:[{id:`nice-move`,text:`Nice move`},{id:`careful`,text:`Careful`},{id:`finish-it`,text:`Finish it`},{id:`wait`,text:`Wait`}]}],et=new Map($e.flatMap(e=>e.signals.map(t=>[t.id,{...t,categoryId:e.id,categoryLabel:e.label}]))),tt=56;function nt(e){return et.get(e)??null}function rt(e,t){return!e||!t?null:e.teams.alpha?.playerIds.includes(t)?z.ALPHA:e.teams.beta?.playerIds.includes(t)?z.BETA:null}function it(e,t){let n=rt(e,t);if(!e||!n||!t)return[];let r=e.teams[n]?.playerIds.filter(e=>e!==t)??[];return e.players.filter(e=>r.includes(e.id))}function at(e,t){return!!(e?.source===`realtime`&&it(e,t).length)}var ot=class{constructor({store:e,socket:t}){this.store=e,this.socket=t,this.hideTimer=null,this.unsubscribe=this.socket.on(`receive_signal`,e=>this.handleIncomingSignal(e))}destroy(){this.clearHideTimer(),this.unsubscribe?.()}clearHideTimer(){this.hideTimer&&=(window.clearTimeout(this.hideTimer),null)}stripSignalToasts(e){let t=new Set(Array.from(et.values(),e=>e.text));return(e??[]).filter(e=>{let n=String(e?.message??``).trim(),r=n.indexOf(`:`);if(r<=0)return!0;let i=n.slice(r+1).trim();return!t.has(i)})}showSignal(e){let t=this.store.getState();this.clearHideTimer(),this.store.dispatch({type:G.PATCH_UI,payload:{incomingSignal:{signalId:e.signalId,categoryId:e.categoryId,text:e.text,fromPlayerId:e.fromPlayerId,fromPlayerName:e.fromPlayerName,receivedAt:e.receivedAt??Date.now()},signalHighlightUntil:Date.now()+U.SIGNAL_DISPLAY,toasts:this.stripSignalToasts(t.ui.toasts)}}),this.hideTimer=window.setTimeout(()=>{this.store.dispatch({type:G.PATCH_UI,payload:{signalHighlightUntil:0}}),this.hideTimer=null},U.SIGNAL_DISPLAY)}pushToast(e,t=`info`){this.store.dispatch({type:G.PUSH_TOAST,payload:{id:q(`toast`),message:e,tone:t}})}openSheet(){let e=this.store.getState();if(!(!e.match||e.route!==F.MATCH)){if(!at(e.room,e.session.localPlayerId)){this.pushToast(`Signals are available only in team multiplayer rooms.`,`info`);return}this.store.dispatch({type:G.PATCH_UI,payload:{signalSheetOpen:!0}})}}closeSheet(){this.store.dispatch({type:G.PATCH_UI,payload:{signalSheetOpen:!1}})}sanitizeCustomText(e){return String(e??``).replace(/\s+/g,` `).trim().slice(0,tt)}sendSignal(e){let t=this.store.getState(),n=nt(e),r=Number(t.ui.signalCooldownUntil)||0;if(!n||!t.match||!t.room||!at(t.room,t.session.localPlayerId)){this.closeSheet();return}if(Date.now()<r){let e=Math.max(1,Math.ceil((r-Date.now())/1e3));this.pushToast(`Signals cooling down. Try again in ${e}s.`,`info`),this.closeSheet();return}let i=Date.now()+U.SIGNAL_COOLDOWN;this.store.dispatch({type:G.PATCH_UI,payload:{signalSheetOpen:!1,signalCooldownUntil:i}}),this.socket.emit(`send_signal`,{signalId:n.id},e=>{if(e?.ok){this.showSignal({signalId:n.id,categoryId:n.categoryId,text:n.text,fromPlayerId:t.session.localPlayerId??null,fromPlayerName:`You`});return}this.pushToast(e?.error??`Unable to send teammate signal.`,`warning`)})}sendCustomSignal(e){let t=this.store.getState(),n=Number(t.ui.signalCooldownUntil)||0,r=this.sanitizeCustomText(e);if(!t.match||!t.room||!at(t.room,t.session.localPlayerId)){this.closeSheet();return}if(!r){this.pushToast(`Type a short teammate note first.`,`info`);return}if(Date.now()<n){let e=Math.max(1,Math.ceil((n-Date.now())/1e3));this.pushToast(`Signals cooling down. Try again in ${e}s.`,`info`);return}let i=Date.now()+U.SIGNAL_COOLDOWN;this.store.dispatch({type:G.PATCH_UI,payload:{signalSheetOpen:!1,signalCooldownUntil:i}}),this.socket.emit(`send_signal`,{customText:r},e=>{if(e?.ok){this.showSignal({signalId:`custom`,categoryId:`custom`,text:r,fromPlayerId:t.session.localPlayerId??null,fromPlayerName:`You`});return}this.pushToast(e?.error??`Unable to send teammate note.`,`warning`)})}handleIncomingSignal(e){let t=this.store.getState(),n=t.session.localPlayerId,r=rt(t.room,n),i=e?.senderTeamId===z.ALPHA||e?.senderTeamId===z.BETA?e.senderTeamId:null;!t.room||e?.roomId!==t.room.code||e?.fromPlayerId&&n&&e.fromPlayerId===n||r&&i&&r!==i||this.showSignal({signalId:e?.signalId??``,categoryId:e?.categoryId??`general`,text:e?.text??`Signal`,fromPlayerId:e?.fromPlayerId??null,fromPlayerName:e?.fromPlayerName??`Teammate`,receivedAt:e?.sentAt??Date.now()})}};function st(e,t){let n=t.revealState;return e.ui.isPaused?{kicker:`Paused`,title:`Match paused`,note:`Selections are frozen until the match resumes.`,toneClass:`is-warning`}:e.connection.status===`reconnecting`?{kicker:`Reconnect`,title:`Restoring link`,note:`Waiting for the realtime connection to recover.`,toneClass:`is-warning`}:n.status===`locked`?{kicker:`Syncing`,title:`Both picks locked`,note:`Numbers stay hidden until the reveal animation completes.`,toneClass:`is-live`}:n.status===`revealing`?{kicker:`Reveal`,title:n.lastBall?.commentary??`Ball resolved`,note:`Both numbers are shown together to keep every ball fair.`,toneClass:`is-live`}:n.status===`waiting`?{kicker:`Waiting`,title:`Waiting for opponent`,note:e.ui.connectionBanner||`One side has locked in. Waiting for the other pick.`,toneClass:``}:{kicker:`Ready`,title:n.lastBall?.commentary??`Awaiting next ball`,note:`Lock both picks to start the next reveal.`,toneClass:``}}function ct(e,t,n,r,i,a){let o=t.innings[t.currentInningsIndex];return e.ui.isPaused?{label:`Match paused`,tone:`is-warning`}:e.connection.status===`reconnecting`?{label:`Reconnecting`,tone:`is-warning`}:t.revealState.status===`locked`?{label:`Reveal syncing`,tone:`is-live`}:t.revealState.status===`revealing`?{label:`Result live`,tone:`is-live`}:i?t.revealState.waitingFor===`batting`?{label:`Batting side to pick`,tone:``}:t.revealState.waitingFor===`bowling`?{label:`Bowling side to pick`,tone:``}:{label:`Pick both sides`,tone:`is-live`}:t.revealState.waitingFor?t.revealState.waitingFor===n?{label:`Your pick needed`,tone:`is-live`}:a&&r&&o[t.revealState.waitingFor===`batting`?`battingTeamId`:`bowlingTeamId`]===r?{label:t.revealState.waitingFor===`batting`?`AI teammate choosing`:`AI teammate bowling`,tone:``}:{label:`Waiting for opponent`,tone:``}:n===`batting`?{label:`Your turn to bat`,tone:`is-live`}:n===`bowling`?{label:`Your turn to bowl`,tone:`is-live`}:a&&r&&(o.battingTeamId===r||o.bowlingTeamId===r)?{label:`AI teammate on this ball`,tone:``}:{label:`Waiting for active players`,tone:``}}function lt(t){if(!t.room||!t.match)return``;let n=Object.fromEntries(t.room.players.map(e=>[e.id,e])),r=t.match.innings[t.match.currentInningsIndex],i=t.session.localPlayerId,a=t.room.teams.alpha?.playerIds.includes(i??``)?`alpha`:t.room.teams.beta?.playerIds.includes(i??``)?`beta`:null,o=r.strikerId===i?`batting`:r.currentBowlerId===i?`bowling`:null,s=t.match.settings.controlMode===`hotseat`,c=t.room.source===`offline`,l=at(t.room,i),u=it(t.room,i),d=Number(t.ui.signalCooldownUntil)||0,f=Date.now()<d,p=Math.max(1,Math.ceil((d-Date.now())/1e3)),m=t.ui.incomingSignal??null,h=Number(t.ui.signalHighlightUntil)||0,g=!!m&&Date.now()<h,_=l||!!m,v=!!t.ui.isPaused,y=t.match.revealState,ee=r.pendingChoices.batting?.value??null,te=r.pendingChoices.bowling?.value??null,ne=t.ui.selectedPicks??{batting:null,bowling:null},b=Array.isArray(t.match.settings.allowedNumbers)&&t.match.settings.allowedNumbers.length?t.match.settings.allowedNumbers:[1,2,3,4,5,6],x=t.match.settings.numberSetLabel??`Classic 1-6`,S=y.lockedSides.includes(`batting`)||ee!==null,C=y.lockedSides.includes(`bowling`)||te!==null,w=S?ee:ne.batting,T=C?te:ne.bowling,re=v||y.status===`locked`||y.status===`revealing`||t.connection.status===`reconnecting`||t.connection.status===`offline`,ie=re||(s?S:o!==`batting`||S),E=re||(s?C:o!==`bowling`||C),D=t.match.teams[r.battingTeamId],O=t.match.teams[r.bowlingTeamId],k=D.battingOrder.filter(e=>![r.strikerId,r.nonStrikerId,...r.dismissedIds].includes(e)),A=s||(a?r.battingTeamId===a:!1),ae=s||(a?r.bowlingTeamId===a:!1),j=v||y.status===`locked`||y.status===`revealing`||!!(r.pendingChoices.batting||r.pendingChoices.bowling)||t.match.settings.bowlingMode===`over-locked`&&r.scoreboard.ballsInOver!==0,M=st(t,t.match),oe=ct(t,t.match,o,a,s,c),se=le(t.match,t.match.ballEvents,b),ce=ue(t.match,t.match.ballEvents,b),fe=de(t.match,t.match.ballEvents,b,3),N=y.status===`revealing`,P=t.match.revealState.lastBall,pe=N?P?.resultType===`wicket`?`is-wicket`:P?.resultTone===`mega`?`is-mega`:P?.resultTone===`boundary`||P?.resultTone===`power`?`is-boundary`:`is-run`:``,me=N?P?.resultLabel??(P?.resultType===`wicket`?`Wicket!`:`+${P?.runs??0}`):``,F=N?P?.battingNumber??`-`:S||y.status===`locked`?`Locked`:`Waiting`,I=N?P?.bowlingNumber??`-`:C||y.status===`locked`?`Locked`:`Waiting`;return`
    <section class="match-layout">
      ...
      <section class="tactical-strip" aria-label="Mind game hints from revealed balls">
        <div class="tactical-chip tactical-chip--${se.tone}">
          <span>Pressure</span>
          <strong>${se.label}</strong>
          <small>${se.detail}</small>
        </div>
        ...
        <details class="mind-hints" ${fe.length?`open`:``}>
          <summary>Mind Game</summary>
          ${fe.length?`<ul>${fe.map(e=>`<li>${e}</li>`).join(``)}</ul>`:`<p class="muted">Reveal a few balls to build useful patterns.</p>`}
        </details>
      </section>
      ${K({kicker:`Team Signal`,title:_?`Quick coordination`:`Signals unavailable`,className:g?`card--focus`:``,content:_?`
      ...
      <div class="match-layout__controls">
        ${K({kicker:M.kicker,title:M.title,className:`card--focus`,content:`
            <div class="round-banner ${M.toneClass}">
              <strong>${t.ui.connectionBanner||M.title}</strong>
              <span>${M.note}</span>
            </div>
            <div class="number-pad-row">
              ${Ye({title:`Batting Pick`,side:`batting`,selected:w,disabled:ie,locked:S,busy:y.status===`locked`||y.status===`revealing`,waiting:S&&y.status===`waiting`,allowedNumbers:b,statusLabel:S?`Locked`:ie?`Waiting`:`Ready`,note:s?`Pick for batting side`:o===`batting`?S?`Your batting pick is locked in.`:`Your side is batting`:c&&a===r.battingTeamId?S?`AI teammate locked batting pick`:`AI teammate is batting`:S?`Opponent locked batting pick`:`Waiting for opponent`})}
              ${Ye({title:`Bowling Pick`,side:`bowling`,selected:T,disabled:E,locked:C,busy:y.status===`locked`||y.status===`revealing`,waiting:C&&y.status===`waiting`,allowedNumbers:b,statusLabel:C?`Locked`:E?`Waiting`:`Ready`,note:s?`Pick for bowling side`:o===`bowling`?C?`Your bowling pick is locked in.`:`Your side is bowling`:c&&a===r.bowlingTeamId?C?`AI teammate locked bowling pick`:`AI teammate is bowling`:C?`Opponent locked bowling pick`:`Waiting for opponent`})}
            </div>
            <div class="action-row">
        ...
        ${K({kicker:y.status===`revealing`?`Reveal`:y.status===`locked`?`Hidden Picks`:y.status===`waiting`?`Waiting`:`Latest Ball`,title:y.status===`revealing`?t.match.revealState.lastBall?.commentary??`Ball resolved`:y.status===`locked`?`Both picks are hidden`:y.status===`waiting`?`Waiting for opponent`:t.match.revealState.lastBall?.commentary??`Awaiting first reveal`,content:`
            <div class="reveal-panel ${N?`is-live`:``} ${y.status===`locked`?`is-hidden`:``}">
              <div>
                <strong>Bat</strong>
              ...
              <div>
                <strong>Bowl</strong>
                <span class="reveal-panel__value">${I}</span>
              </div>
            </div>
            ${N?`<div class="ball-result ${pe}">${me}</div>`:``}
            ${N&&P?.commentary?`<p class="result-copy">${P.commentary}</p>`:``}
      ...
      <div class="grid-two">
        ${K({kicker:`Captain Control`,title:`Batting Order Management`,content:A?He({title:D.name,teamId:r.battingTeamId,type:`batting`,order:k,playersById:n,movableIds:k,helper:`Only remaining batters can be reshuffled mid-match.`}):`<p class="muted">Opponent captain is managing the remaining batting queue.</p>`})}
        ${K({kicker:`Captain Control`,title:`Bowling Order And Changes`,content:ae?`
                <p class="muted">
                  ${j&&t.match.settings.bowlingMode===`over-locked`&&r.scoreboard.ballsInOver!==0?`Bowler is locked until the over ends.`:j?`Bowler changes unlock after the current ball resolves.`:`Tap a bowler to make them active for the next ball.`}
                </p>
                ${He({title:`${O.name} bowling queue`,teamId:r.bowlingTeamId,type:`bowling`,order:O.bowlingOrder,playersById:n,locked:j,helper:`Top position becomes the active bowler when changes are allowed.`})}
                <div class="chip-row">
                  ${O.playerIds.map(e=>{let t=n[e];return`
                        <button
      ...
      ${Je(t.match.ballEvents,{match:t.match,allowedNumbers:b})}
  ...
  `:``}function dt(t){if(!t.match?.result||!t.room)return ut(t.lastSavedMatch);let n=Object.fromEntries(t.room.players.map(e=>[e.id,e])),r=t.match.innings.reduce((e,t)=>e+t.scoreboard.runs,0),i=t.match.innings.reduce((e,t)=>e+t.scoreboard.wickets,0),a=t.match.innings.reduce((e,t)=>e+t.scoreboard.legalBalls,0),o=Array.isArray(t.match.settings.allowedNumbers)&&t.match.settings.allowedNumbers.length?t.match.settings.allowedNumbers:[1,2,3,4,5,6],s=N(t.match,t.match.ballEvents,o),c=t.match.result.winnerTeamId&&t.match.teams[t.match.result.winnerTeamId]?t.match.teams[t.match.result.winnerTeamId]:null,l=e=>`
    ...
    <section class="page-stack">
      ...
      <div class="grid-two">
        ...
        ${K({kicker:`Ball By Ball`,title:`Match Log`,className:`card--span-2`,content:`
            <div class="history-list">
              ${t.match.ballEvents.map((e,n)=>{let r=fe(e,t.match.ballEvents.slice(0,n),o);return`
                      <article class="history-item">
                        <div>
  ...
  `}function ft(t){if(!t.room)return K({title:`No active room`,content:`
    ...
    <section class="page-stack">
      ${K({kicker:`Room Setup`,title:`Room ${t.room.code}`,className:`card--hero`,content:`
        ...
        `})}
      <div class="grid-two">
        ${K({kicker:`Choose Sides`,title:`Team Placement`,className:`card--span-2`,content:Ke({room:t.room,localPlayerId:t.session.localPlayerId})})}
        ${K({kicker:`Presence`,title:`Players Joining`,content:Ge({players:t.room.players,captains:t.room.captains,teams:t.room.teams,hostId:t.room.hostId,localPlayerId:t.session.localPlayerId,canRemovePlayers:i&&!a})})}
  ...
  `}function gt(t){let n=t.room?.tossResult??null;if(!t.room?.teams.alpha||!t.room.teams.beta)return``;let r=t.session.localPlayerId,i=t.room.captains.alpha===r,a=t.room.captains.beta===r,o=i||a,s=n?t.room.captains[n.winnerTeamId]:null,c=!!(n&&!n.decision&&s===r),l=n?t.room.teams[n.winnerTeamId]?.name??n.winnerTeamId:``,u=`
  ...
  `}var _t={[F.HOME]:Ve,[F.ROOM]:ft,[F.LOBBY]:qe,[F.DRAFT]:Re,[F.TOSS]:gt,[F.LINEUP]:Ue,[F.MATCH]:lt,[F.RESULT]:dt,[F.HISTORY]:ze,[F.RULES]:ht};function vt(e,t,n){let r=[...e],i=n===`up`?t-1:t+1;return i<0||i>=r.length||([r[t],r[i]]=[r[i],r[t]]),r}var yt=class{constructor({root:e,store:t,roomService:n,matchService:r,signalSystem:i,socket:a}){this.root=e,this.store=t,this.roomService=n,this.matchService=r,this.signalSystem=i,this.socket=a,this.renderedRoute=``}mount(){this.root.addEventListener(`click`,e=>this.handleClick(e)),this.root.addEventListener(`change`,e=>this.handleChange(e)),this.root.addEventListener(`input`,e=>this.handleInput(e)),this.root.addEventListener(`keydown`,e=>this.handleKeyDown(e)),this.store.subscribe(()=>this.render()),this.render()}render(){let e=this.store.getState(),t=(_t[e.route]??_t.home)(e),n=i({route:e.route,content:t,connection:e.connection,ui:e.ui}),r=()=>{this.root.innerHTML=n,this.renderedRoute=e.route},a=document;if(this.renderedRoute&&this.renderedRoute!==e.route&&typeof a.startViewTransition==`function`){a.startViewTransition(()=>{r()});return}r()}handleClick(e){let t=e.target instanceof HTMLElement?e.target.closest(`[data-action]`):null;if(!t)return;let n=t.dataset.action,r=this.store.getState();switch(n){case`navigate`:this.store.dispatch({type:G.NAVIGATE,payload:t.dataset.route||`home`});break;case`quick-match`:this.roomService.startQuickMatch({teamSize:r.ui.playersPerTeam});break;case`create-room`:this.roomService.createRoom({playerName:r.session.profileName,teamSize:r.ui.playersPerTeam});break;case`join-room`:this.roomService.joinRoom({playerName:r.session.profileName,code:r.ui.roomCodeInput.trim().toUpperCase()});break;case`practice-mode`:this.roomService.startPracticeMode();break;case`enter-lobby`:this.roomService.enterLobby();break;case`toggle-ready`:this.roomService.toggleReady();break;case`copy-invite`:this.roomService.copyInviteLink();break;case`remove-player`:this.roomService.removePlayer(t.dataset.player||``);break;case`set-player-team`:this.roomService.setPlayerTeam(t.dataset.player||``,t.dataset.team||null);break;case`discard-room`:if(!r.room)break;this.store.dispatch({type:G.PATCH_UI,payload:{confirmDialog:{title:r.room.source===`offline`?`Discard Practice Room`:`Discard Room`,message:r.room.source===`offline`?`Discard this practice room and return home?`:`Discard room ${r.room.code}? This will remove every player from the room.`,confirmLabel:`Discard`,cancelLabel:`Keep Room`,intent:`discard-room`}}});break;case`cancel-dialog`:this.store.dispatch({type:G.PATCH_UI,payload:{confirmDialog:null}});break;case`confirm-dialog`:{let e=r.ui.confirmDialog?.intent??null;this.store.dispatch({type:G.PATCH_UI,payload:{confirmDialog:null}}),e===`discard-room`&&(this.matchService.clearTimers(),this.roomService.discardRoom());break}case`auto-draft`:this.roomService.autoDraftTeams();break;case`open-draft`:this.roomService.openDraftBoard();break;case`open-signal-sheet`:this.signalSystem.openSheet();break;case`close-signal-sheet`:this.signalSystem.closeSheet();break;case`send-signal`:this.signalSystem.sendSignal(t.dataset.signal||``);break;case`send-custom-signal`:{let e=this.root.querySelector(`[data-signal-input='custom']`);this.signalSystem.sendCustomSignal(e?.value??``);break}case`draft-pick`:this.roomService.pickDraftPlayer(t.dataset.player||``);break;case`start-toss`:this.matchService.startToss();break;case`run-toss`:this.matchService.runToss(t.dataset.call||`heads`);break;case`choose-toss`:this.matchService.chooseTossDecision(t.dataset.decision||`bat`);break;case`start-match`:this.roomService.lockLineups(),this.matchService.confirmLineupsAndStart();break;case`move-lineup`:this.handleLineupMove(t);break;case`pick-number`:if(!r.ui.isPaused){let e=t.dataset.side||`batting`;this.matchService.submitSelection(e,Number(t.dataset.value),{previewOnly:!0})}break;case`lock-number`:if(!r.ui.isPaused){let e=t.dataset.side||`batting`,n=r.ui.selectedPicks?.[e];this.matchService.submitSelection(e,Number(n))}break;case`apply-number-set`:this.roomService.updateSettings(r.ui.numberSetDraft??r.room?.settings??{});break;case`choose-number-preset`:this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:{...r.ui.numberSetDraft??r.room?.settings??{},numberSetMode:`preset`,numberSetPreset:t.dataset.preset||`classic`},numberSetValidationError:``}});break;case`change-bowler`:this.matchService.changeBowler(t.dataset.team||``,t.dataset.player||``);break;case`toggle-pause`:this.matchService.togglePause();break;case`simulate-reconnect`:this.matchService.simulateReconnect();break;case`back-home`:this.matchService.clearTimers(),this.roomService.resetToHome();break;case`dismiss-toast`:this.store.dispatch({type:G.DISMISS_TOAST,payload:t.dataset.id});break;default:break}}handleLineupMove(e){let t=e.dataset.team||`alpha`,n=e.dataset.type||`batting`,r=Number(e.dataset.index),i=e.dataset.direction||`up`,a=this.store.getState();if(a.route===`lineup`&&a.room?.teams[t]){let e=a.room.teams[t][n===`batting`?`battingOrder`:`bowlingOrder`];this.roomService.updateTeamLineup(t,n,vt(e,r,i));return}if(a.route===`match`&&a.match){let e=a.match.teams[t];if(n===`bowling`){this.matchService.reorderBowlers(t,vt(e.bowlingOrder,r,i));return}let o=a.match.innings[a.match.currentInningsIndex],s=e.battingOrder.filter(e=>![o.strikerId,o.nonStrikerId,...o.dismissedIds].includes(e));this.matchService.reorderBatters(t,vt(s,r,i))}}handleChange(e){let t=e.target instanceof HTMLElement?e.target:null;if(!t?.dataset.field)return;let n=t.dataset.field,r=t.value,i=this.store.getState();switch(n){case`profile-name`:this.store.dispatch({type:G.UPDATE_SESSION,payload:{profileName:r}});break;case`room-code`:this.store.dispatch({type:G.PATCH_UI,payload:{roomCodeInput:r.toUpperCase()}});break;case`match-mode`:this.roomService.updateSettings({matchMode:r});break;case`bowling-mode`:this.roomService.updateSettings({bowlingMode:r});break;case`overs`:this.roomService.updateSettings({overs:Number(r)});break;case`ai-difficulty`:this.roomService.updateSettings({aiDifficulty:r});break;case`number-set-mode`:this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:{...i.ui.numberSetDraft??i.room?.settings??{},numberSetMode:r},numberSetValidationError:``}});break;case`number-set-preset`:this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:{...i.ui.numberSetDraft??i.room?.settings??{},numberSetMode:`preset`,numberSetPreset:r},numberSetValidationError:``}});break;case`number-range-min`:this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:{...i.ui.numberSetDraft??i.room?.settings??{},numberSetMode:`range`,numberRangeMin:Number(r)},numberSetValidationError:``}});break;case`number-range-max`:this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:{...i.ui.numberSetDraft??i.room?.settings??{},numberSetMode:`range`,numberRangeMax:Number(r)},numberSetValidationError:``}});break;case`custom-numbers`:this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:{...i.ui.numberSetDraft??i.room?.settings??{},numberSetMode:`custom`,customNumbersText:r},numberSetValidationError:``}});break;case`players-per-team`:this.store.dispatch({type:G.PATCH_UI,payload:{playersPerTeam:Number(r)}});break;case`captain-alpha`:this.roomService.setCaptain(`alpha`,r);break;case`captain-beta`:this.roomService.setCaptain(`beta`,r);break;case`team-name-alpha`:this.roomService.updateTeamName(`alpha`,r);break;case`team-name-beta`:this.roomService.updateTeamName(`beta`,r);break;default:break}}handleInput(e){let t=(e.target instanceof HTMLElement?e.target:null)?.dataset.field??``;(t===`number-range-min`||t===`number-range-max`||t===`custom-numbers`)&&this.handleChange(e)}handleKeyDown(e){let t=e.target instanceof HTMLElement?e.target:null;if(!t?.matches(`[data-signal-input='custom']`)||e.key!==`Enter`||e.shiftKey)return;e.preventDefault();let n=t;this.signalSystem.sendCustomSignal(n?.value??``)}};function bt(e,t){let n=!1,r=e=>{let n=e.replace(/^#\/?/,``)||`home`;return t[n]?n:`home`};window.addEventListener(`hashchange`,()=>{n||e.dispatch({type:G.NAVIGATE,payload:r(window.location.hash)})}),e.subscribe(e=>{let t=`#/${e.route}`;window.location.hash!==t&&(n=!0,window.location.hash=t,window.setTimeout(()=>{n=!1},0))}),e.dispatch({type:G.NAVIGATE,payload:r(window.location.hash)})}function xt(){let e=De(),t=Me(),n=typeof window<`u`?new URLSearchParams(window.location.search).get(`room`)?.trim().toUpperCase()??``:``;return{route:F.HOME,session:e,ui:{roomCodeInput:n||e.lastRoomCode||``,tossCall:`heads`,playersPerTeam:V.playersPerTeam,connectionBanner:`Realtime room service ready`,selectedNumberSide:null,selectedPicks:{batting:null,bowling:null},numberSetDraft:null,numberSetValidationError:``,isPaused:!1,signalSheetOpen:!1,signalCooldownUntil:0,signalHighlightUntil:0,incomingSignal:null,confirmDialog:null,toasts:[]},connection:{status:R.RECONNECTING,lastSyncAt:null,latencyMs:28,note:`Connecting to realtime backend`},room:null,match:null,history:ke(),lastSavedMatch:t}}function St(e,t){let n=e,r=new Set;return{getState(){return n},subscribe(e){return r.add(e),()=>r.delete(e)},dispatch(e){return n=t(n,e),r.forEach(e=>e(n)),e}}}function Ct(e,t){return e===t}function wt(e){return e%2==1}function Tt(e,t){return Math.max(t*6-e,0)}function Et(e,t){return e>=Math.max(t,1)}function Dt(e,t){return e.length===t.length&&e.every(e=>t.includes(e))&&new Set(e).size===e.length}function X(e,t=y().allowedNumbers){return C(e,t)}function Ot(e,t,n){if(e.phase!==L.LIVE)return!1;let r=e.innings[e.currentInningsIndex],i=r.bowlingTeamId===`alpha`?`alpha`:`beta`,a=e.teams[i];return!(r.bowlingTeamId!==t||!a.playerIds.includes(n)||r.pendingChoices.batting||r.pendingChoices.bowling||e.settings.bowlingMode===ge.LOCKED&&r.scoreboard.ballsInOver!==0)}function kt(e,t,n){if(e.phase!==L.LIVE)return!1;let r=e.innings[e.currentInningsIndex];if(r.battingTeamId!==t)return!1;let i=e.teams[t===`alpha`?`alpha`:`beta`],a=[r.strikerId,r.nonStrikerId,...r.dismissedIds].filter(Boolean),o=i.battingOrder.filter(e=>!a.includes(e));return o.length===n.length&&n.every(e=>o.includes(e))&&new Set(n).size===n.length}function At(e,t,n){if(e.phase!==L.LIVE)return!1;let r=e.innings[e.currentInningsIndex],i=e.teams[t===`alpha`?`alpha`:`beta`];return!(r.bowlingTeamId!==t||!Dt(n,i.playerIds)||r.pendingChoices.batting||r.pendingChoices.bowling||e.settings.bowlingMode===ge.LOCKED&&r.scoreboard.ballsInOver!==0)}function jt(e){let t=Object.fromEntries(e.playerIds.map(e=>[e,{runs:0,ballsFaced:0,wicketsTaken:0,ballsBowled:0,dismissals:0}]));return{...a(e),currentBowlerId:null,stats:t}}function Z(e,t){return t===z.ALPHA?e.alpha:e.beta}function Q(e){return e.innings[e.currentInningsIndex]}function Mt(e,t){let n=[];return X(e.pendingChoices.batting?.value??null,t)&&n.push(`batting`),X(e.pendingChoices.bowling?.value??null,t)&&n.push(`bowling`),n}function Nt(e={}){return{status:`idle`,waitingFor:null,lockedSides:[],revealAt:null,cycleId:0,lastBall:null,...e}}function Pt(e){return e.phase===L.LIVE&&!Q(e).completed}function Ft(e,t){e.overs=Math.floor(e.legalBalls/6),e.ballsLeft=Tt(e.legalBalls,t.overs),e.target&&(e.requiredRuns=Math.max(e.target-e.runs,0))}function It(e,t){t.matchMode===he.TWO_BATSMEN&&(!e.strikerId||!e.nonStrikerId||([e.strikerId,e.nonStrikerId]=[e.nonStrikerId,e.strikerId]))}function Lt(e,t){let n=t.battingOrder[e.nextBatterIndex]??null;return n?(e.nextBatterIndex+=1,e.strikerId=n,n):e.nonStrikerId?(e.strikerId=e.nonStrikerId,e.nonStrikerId=null,e.strikerId):(e.strikerId=null,null)}function Rt(e,t){let n=t.bowlingOrder;if(!n.length)return e.currentBowlerId=null,t.currentBowlerId=null,null;let r=n[e.nextBowlerIndex%n.length]??n[0];return e.currentBowlerId=r,t.currentBowlerId=r,e.nextBowlerIndex=(e.nextBowlerIndex+1)%n.length,r}function zt(e,t,n,r,i){let a=Z(e,t),o=Z(e,n),s=a.battingOrder[0]??null,c=r.matchMode===he.TWO_BATSMEN?a.battingOrder[1]??null:null,l=o.bowlingOrder[0]??null;o.currentBowlerId=l;let u={battingTeamId:t,bowlingTeamId:n,scoreboard:{runs:0,wickets:0,legalBalls:0,overs:0,ballsInOver:0,target:i,requiredRuns:i,ballsLeft:r.overs*6},strikerId:s,nonStrikerId:c,currentBowlerId:l,nextBatterIndex:r.matchMode===he.TWO_BATSMEN?2:1,nextBowlerIndex:1,dismissedIds:[],completed:!1,pendingChoices:{batting:null,bowling:null}};return Ft(u.scoreboard,r),u}function Bt(e,t){let n=t.scoreboard.runs+1,r=e.innings[1];r.scoreboard.target=n,r.scoreboard.requiredRuns=n,r.scoreboard.ballsLeft=e.settings.overs*6,Ft(r.scoreboard,e.settings),e.currentInningsIndex=1,e.phase=L.BREAK}function Vt(e){let t=a(e),n=t.innings[0],r=t.innings[1],i=n.battingTeamId===z.ALPHA?n.scoreboard.runs:r.scoreboard.runs,o=n.battingTeamId===z.BETA?n.scoreboard.runs:r.scoreboard.runs,s;if(i===o)s={winnerTeamId:null,marginText:`Match tied`,reason:`Scores level`,summary:`Both teams finished on the same score.`,isTie:!0};else{let e=i>o?z.ALPHA:z.BETA,n=e===z.ALPHA?z.BETA:z.ALPHA,r=Z(t.teams,e),a=Z(t.teams,n),c=t.innings[1],l=!!(c.scoreboard.target&&c.scoreboard.runs>=c.scoreboard.target),u=Math.max(r.playerIds.length-c.scoreboard.wickets,0),d=Math.abs(i-o);s={winnerTeamId:e,marginText:l?`${r.name} won by ${u} wicket${u===1?``:`s`}`:`${r.name} won by ${d} run${d===1?``:`s`}`,reason:l?`Target chased`:`Defended total`,summary:`${r.name} beat ${a.name}.`,isTie:!1}}return t.result=s,t.phase=L.COMPLETE,t.updatedAt=Date.now(),t}function Ht(e){if(!Pt(e))return e;let t=a(e),n=Q(t),r=Z(t.teams,n.bowlingTeamId);return n.scoreboard.ballsInOver<6?t:(n.scoreboard.ballsInOver=0,It(n,t.settings),Rt(n,r),t.updatedAt=Date.now(),t)}function Ut(e){let t=a(e),n=Q(t),r=Z(t.teams,n.battingTeamId),i=Et(n.scoreboard.wickets,r.playerIds.length),o=n.scoreboard.legalBalls>=t.settings.overs*6,s=!!(n.scoreboard.target&&n.scoreboard.runs>=n.scoreboard.target);return!i&&!o&&!s?(t.phase=L.LIVE,t.updatedAt=Date.now(),t):(n.completed=!0,t.currentInningsIndex===0?(Bt(t,n),t.updatedAt=Date.now(),t):Vt(t))}function Wt(e){let t=a(e);return t.phase=L.LIVE,t.revealState=Nt({cycleId:t.revealState.cycleId+1,lastBall:t.revealState.lastBall}),t.updatedAt=Date.now(),t}function Gt(e,t){if(!e.teams.alpha||!e.teams.beta)throw Error(`Cannot start match without two drafted teams.`);let n=S(e.settings,e.settings),r={...a(e.settings),...n.ok?n.settings:S({},{}).settings},i={alpha:jt(e.teams.alpha),beta:jt(e.teams.beta)},o=t.decision===`bat`?t.winnerTeamId:t.winnerTeamId===z.ALPHA?z.BETA:z.ALPHA,s=o===z.ALPHA?z.BETA:z.ALPHA;return{id:q(`match`),roomId:e.id,phase:L.LIVE,settings:r,teams:i,tossResult:a(t),innings:[zt(i,o,s,r,null),zt(i,s,o,r,null)],currentInningsIndex:0,revealState:Nt(),ballEvents:[],result:null,updatedAt:Date.now()}}function Kt(e,t,n){let r=a(e),i=Q(r);i.pendingChoices[t]=n;let o=Mt(i,r.settings.allowedNumbers);return r.revealState={...r.revealState,status:o.length===2?`locked`:o.length===1?`waiting`:`idle`,waitingFor:o.length===1?o[0]===`batting`?`bowling`:`batting`:null,lockedSides:o,revealAt:null},r.updatedAt=Date.now(),r}function qt(e,t={}){let n=a(e),r=Mt(Q(n),n.settings.allowedNumbers);return n.revealState={...n.revealState,lockedSides:r,waitingFor:r.length===1?r[0]===`batting`?`bowling`:`batting`:null,...t},n.updatedAt=Date.now(),n}function Jt(e){let t=a(e);return t.revealState=Nt({cycleId:t.revealState.cycleId+1,lastBall:t.revealState.lastBall}),t.updatedAt=Date.now(),t}function Yt(e){let t=a(e),n=Q(t);return n.pendingChoices={batting:null,bowling:null},t.revealState=Nt({cycleId:t.revealState.cycleId+1,lastBall:t.revealState.lastBall}),t.updatedAt=Date.now(),t}function Xt(e,t,n){if(!Pt(e)||!X(t,e.settings.allowedNumbers)||!X(n,e.settings.allowedNumbers))return e;let r=a(e),i=Q(r),o=Z(r.teams,i.battingTeamId),s=Z(r.teams,i.bowlingTeamId),c=i.strikerId,l=i.nonStrikerId,u=i.currentBowlerId,d=Math.floor(i.scoreboard.legalBalls/6),f=i.scoreboard.ballsInOver+1,p=Ct(t,n),m=p?0:t,h=ie({battingNumber:t,bowlingNumber:n,runs:m,isWicket:p,allowedNumbers:r.settings.allowedNumbers});if(!c||!u)return Ut(r);o.stats[c].ballsFaced+=1,s.stats[u].ballsBowled+=1,p?(i.scoreboard.wickets+=1,o.stats[c].dismissals+=1,i.dismissedIds.push(c),s.stats[u].wicketsTaken+=1,Lt(i,o)):(i.scoreboard.runs+=m,o.stats[c].runs+=m,wt(m)&&It(i,r.settings)),i.scoreboard.legalBalls+=1,i.scoreboard.ballsInOver+=1,Ft(i.scoreboard,r.settings);let g={inningsIndex:r.currentInningsIndex,over:d,ball:f,battingNumber:t,bowlingNumber:n,resultType:p?`wicket`:m===0?`dot`:`run`,runs:m,shotLabel:h.shotLabel,resultLabel:h.resultLabel,resultTone:h.resultTone,strikerId:c,nonStrikerId:l,bowlerId:u,battingTeamId:i.battingTeamId,bowlingTeamId:i.bowlingTeamId,timestamp:Date.now(),commentary:h.commentary};r.ballEvents.push(g),r.revealState.lastBall=g,i.pendingChoices={batting:null,bowling:null};let _=Ut(r);if(_.phase!==L.LIVE)return _.revealState.lastBall=g,_.updatedAt=Date.now(),_;if(Q(_).scoreboard.ballsInOver===6){let e=Ht(_);return e.revealState.lastBall=g,e.updatedAt=Date.now(),e}return _.revealState.lastBall=g,_.phase=L.LIVE,_.updatedAt=Date.now(),_}function Zt(e,t,n){if(!kt(e,t,n))return e;let r=a(e),i=Q(r),o=Z(r.teams,t),s=new Set([i.strikerId,i.nonStrikerId,...i.dismissedIds].filter(Boolean)),c=[...n];return o.battingOrder=o.battingOrder.map(e=>s.has(e)?e:c.shift()??e),r.updatedAt=Date.now(),r}function Qt(e,t,n){if(!Ot(e,t,n))return e;let r=a(e),i=Q(r),o=Z(r.teams,t);i.currentBowlerId=n,o.currentBowlerId=n;let s=o.bowlingOrder.indexOf(n);return i.nextBowlerIndex=s===-1?i.nextBowlerIndex:(s+1)%o.bowlingOrder.length,r.updatedAt=Date.now(),r}function $t(e,t,n){if(!At(e,t,n))return e;let r=a(e),i=Q(r),o=Z(r.teams,t),s=o.currentBowlerId&&n.includes(o.currentBowlerId)?o.currentBowlerId:n[0]??null,c=s?n.indexOf(s):-1;return o.bowlingOrder=[...n],o.currentBowlerId=s,i.currentBowlerId=s,i.nextBowlerIndex=n.length?(c+1+n.length)%n.length:0,r.updatedAt=Date.now(),r}function en(e,t){return{id:q(`history`),roomCode:t,playedAt:Date.now(),result:a(e.result),tossResult:a(e.tossResult),settings:a(e.settings),numberSetLabel:e.settings.numberSetLabel??`Classic 1-6`,allowedNumbers:[...e.settings.allowedNumbers??[1,2,3,4,5,6]],insights:N(e,e.ballEvents,e.settings.allowedNumbers),inningsSummary:e.innings.map(e=>({teamId:e.battingTeamId,runs:e.scoreboard.runs,wickets:e.scoreboard.wickets,oversText:P(e.scoreboard.legalBalls)}))}}function tn(e){return ye.filter(t=>!e.includes(t))[0]??`Player ${e.length+1}`}function nn(e,t){let n=tn(t);return t.push(n),{id:q(`mock-${e+1}`),name:n,isHost:!1,isLocal:!1,isMock:!0,role:`player`,status:`joining`,avatarSeed:`${n.toLowerCase()}-${e}`,connectionState:{status:`connected`,lastSyncAt:Date.now(),latencyMs:s(26,72),note:`Mock network`}}}function rn(e,t){let n=[t];return Array.from({length:e},(e,t)=>nn(t,n))}function an(){let[e,t]=o(ve).slice(0,2);return{alpha:e,beta:t}}function on(){return s(xe.AI_PICK_MIN,xe.AI_PICK_MAX)}var sn=on;function cn(e){let t=e.filter(e=>e.weight>0),n=t.reduce((e,t)=>e+t.weight,0),r=Math.random()*n;for(let e of t)if(r-=e.weight,r<=0)return e.value;return t[t.length-1]?.value??e[0]?.value??1}function ln(e,t,n){let r=new Set(t.filter(t=>e.includes(t)));return cn(e.map(e=>({value:e,weight:r.has(e)?n:1})))}function un(e,t,n=e.settings.aiDifficulty||`medium`){let r=e.innings[e.currentInningsIndex],i=r.scoreboard.target?r.scoreboard.requiredRuns??0:0,a=[...Array.isArray(e.settings.allowedNumbers)&&e.settings.allowedNumbers.length?e.settings.allowedNumbers:y().allowedNumbers].sort((e,t)=>e-t),o=a.slice(0,Math.max(1,Math.ceil(a.length/2))),s=a.slice(Math.max(0,Math.floor(a.length/2))),l=n===`beginner`||n===`hard`?n:`medium`,u=O(e.ballEvents,`batting`,5),d=O(e.ballEvents,`bowling`,5),f=A(u);if(l===`beginner`)return c(a);if(t===`bowling`){let e=[u.length>=2&&u[u.length-1]===u[u.length-2]?u[u.length-1]:null,f&&f.count>=2?f.number:null,r.scoreboard.ballsLeft<=6?c(s):null].filter(e=>e!=null),t=l===`hard`?.5:.3;return e.length&&Math.random()<t?ln(a,e,l===`hard`?5:3):c(a)}let p=u[u.length-1]??null,m=i>Math.max(12,a[a.length-1]??6),h=!r.scoreboard.target||i<=Math.max(6,a[Math.floor(a.length/2)]??3);return l===`medium`?c(m&&Math.random()<.45?s:h&&Math.random()<.35?[...o,a[Math.floor(a.length/2)]??o[0]]:a):cn(a.map(e=>{let t=j(e,a),n=1;return e===p&&(n*=.45),d.includes(e)&&(n*=.85),m&&(t===`high`||t===`highest`)?n+=3:h&&(t===`low`||t===`mid`)?n+=1.6:t===`highest`&&(n+=.7),{value:e,weight:n}}))}function dn(e){return e.settings.overs<=2?`bowl`:c([`bat`,`bowl`])}function $(e){return e?.source===`offline`}function fn(e,t){return t?!!e.players.find(e=>e.id===t&&!e.isLocal):!1}function pn(e,t){return{call:`heads`,coinFace:`heads`,winnerTeamId:t===z.ALPHA||t===z.BETA?t:z.ALPHA,decision:`bat`}}function mn(e){return{alpha:e.teams.alpha?{id:e.teams.alpha.id,name:e.teams.alpha.name,captainId:e.teams.alpha.captainId,playerIds:[...e.teams.alpha.playerIds],battingOrder:[...e.lineups?.alpha?.battingOrder??e.teams.alpha.battingOrder],bowlingOrder:[...e.lineups?.alpha?.bowlingOrder??e.teams.alpha.bowlingOrder]}:null,beta:e.teams.beta?{id:e.teams.beta.id,name:e.teams.beta.name,captainId:e.teams.beta.captainId,playerIds:[...e.teams.beta.playerIds],battingOrder:[...e.lineups?.beta?.battingOrder??e.teams.beta.battingOrder],bowlingOrder:[...e.lineups?.beta?.bowlingOrder??e.teams.beta.bowlingOrder]}:null}}var hn=1;function gn(e,t){let n=a(e),r=Number(t?.currentInningsIndex),i=Number(t?.legalBalls);if(!Number.isInteger(r)||r<0||r>=n.innings.length)return e;let o=e.innings[e.currentInningsIndex];if(r<e.currentInningsIndex||r===e.currentInningsIndex&&Number.isFinite(i)&&i<o.scoreboard.legalBalls)return e;n.currentInningsIndex=r;let s=n.innings[r],c=Number(t?.score),l=Number(t?.wickets),u=Number(t?.ballsInOver),d=t?.target==null?null:Number(t.target),f=!!t?.inputsLocked?.[t?.strikerId??``],p=!!t?.inputsLocked?.[t?.currentBowlerId??``],m=[];return s.battingTeamId=t?.battingTeamId??s.battingTeamId,s.bowlingTeamId=t?.bowlingTeamId??s.bowlingTeamId,s.strikerId=t?.strikerId??s.strikerId,s.nonStrikerId=t?.nonStrikerId??s.nonStrikerId,s.currentBowlerId=t?.currentBowlerId??s.currentBowlerId,Number.isFinite(c)&&(s.scoreboard.runs=c),Number.isFinite(l)&&(s.scoreboard.wickets=l),Number.isFinite(i)&&(s.scoreboard.legalBalls=i,s.scoreboard.overs=Math.floor(i/6),s.scoreboard.ballsLeft=Math.max(n.settings.overs*6-i,0)),Number.isFinite(u)&&(s.scoreboard.ballsInOver=u),s.scoreboard.target=d,s.scoreboard.requiredRuns=d==null?null:Math.max(d-s.scoreboard.runs,0),s.completed=!!(!t?.inningsSwitched&&s.completed),s.pendingChoices={batting:f?{controller:`manual`,playerId:s.strikerId,teamId:s.battingTeamId,value:hn,lockedAt:Date.now()}:null,bowling:p?{controller:`manual`,playerId:s.currentBowlerId,teamId:s.bowlingTeamId,value:hn,lockedAt:Date.now()}:null},f&&m.push(`batting`),p&&m.push(`bowling`),n.teams[s.bowlingTeamId].currentBowlerId=s.currentBowlerId,n.phase!==L.COMPLETE&&n.revealState.status!==`revealing`&&(n.phase=t?.inningsSwitched?L.BREAK:L.LIVE,n.revealState={...n.revealState,status:m.length===2?`locked`:m.length===1?`waiting`:`idle`,waitingFor:m.length===1?m[0]===`batting`?`bowling`:`batting`:null,lockedSides:m,revealAt:m.length===2?Date.now()+U.REVEAL_LOCK:null}),n.updatedAt=Date.now(),n}function _n(e){let t=Number(e?.over),n=Number(e?.ball);return!Number.isInteger(t)||!Number.isInteger(n)||n<1||n>6?null:t*6+n}function vn(e){let t=e?.resultType===`out`,n=e?.runs??0,r=ie({battingNumber:e?.battingNumber??0,bowlingNumber:e?.bowlingNumber??0,runs:n,isWicket:t,allowedNumbers:e?.allowedNumbers??e?.settings?.allowedNumbers??[]});return{inningsIndex:e?.inningsIndex??0,over:e?.over??0,ball:e?.ball??1,battingNumber:e?.battingNumber??0,bowlingNumber:e?.bowlingNumber??0,resultType:t?`wicket`:n?`run`:`dot`,runs:n,shotLabel:e?.shotLabel??r.shotLabel,resultLabel:e?.resultLabel??r.resultLabel,resultTone:e?.resultTone??r.resultTone,strikerId:e?.strikerId??null,nonStrikerId:e?.nonStrikerId??null,bowlerId:e?.currentBowlerId??null,battingTeamId:e?.battingTeamId??z.ALPHA,bowlingTeamId:e?.bowlingTeamId??z.BETA,timestamp:e?.timestamp??Date.now(),commentary:e?.commentary??r.commentary}}function yn(e,t,n,r){let i=new Set([t,n].filter(Boolean)),a=[];return e.forEach(e=>{a.length>=r||i.has(e)||a.push(e)}),a}function bn(e,t){let n=a(e),r=Array.isArray(t?.innings)?t.innings:[],i=n.settings.overs*6;return n.currentInningsIndex=Number.isInteger(Number(t?.currentInningsIndex))&&Number(t.currentInningsIndex)>=0?Number(t.currentInningsIndex):n.currentInningsIndex,n.ballEvents=Array.isArray(t?.ballHistory)?t.ballHistory.map(e=>vn(e)):n.ballEvents,n.result=t?.result??null,n.phase=t?.status===`complete`||n.result?L.COMPLETE:L.LIVE,r.forEach((e,t)=>{let r=n.innings[t];if(!r)return;let a=n.teams[e?.battingTeamId??r.battingTeamId],o=n.teams[e?.bowlingTeamId??r.bowlingTeamId],s=Number(e?.legalBalls),c=Number(e?.score),l=Number(e?.wickets),u=Number(e?.ballsInOver),d=e?.target==null?null:Number(e.target),f=e?.strikerId??r.strikerId,p=e?.nonStrikerId??r.nonStrikerId,m=e?.currentBowlerId??r.currentBowlerId,h=[f,p].filter(Boolean).length;r.battingTeamId=e?.battingTeamId??r.battingTeamId,r.bowlingTeamId=e?.bowlingTeamId??r.bowlingTeamId,r.strikerId=f,r.nonStrikerId=p,r.currentBowlerId=m,r.completed=!!e?.completed,r.scoreboard.runs=Number.isFinite(c)?c:r.scoreboard.runs,r.scoreboard.wickets=Number.isFinite(l)?l:r.scoreboard.wickets,r.scoreboard.legalBalls=Number.isFinite(s)?s:r.scoreboard.legalBalls,r.scoreboard.overs=Math.floor(r.scoreboard.legalBalls/6),r.scoreboard.ballsInOver=Number.isFinite(u)?u:r.scoreboard.legalBalls%6,r.scoreboard.target=d,r.scoreboard.requiredRuns=d==null?null:Math.max(d-r.scoreboard.runs,0),r.scoreboard.ballsLeft=Math.max(i-r.scoreboard.legalBalls,0),r.dismissedIds=a?yn(a.battingOrder,r.strikerId,r.nonStrikerId,r.scoreboard.wickets):[],r.nextBatterIndex=r.dismissedIds.length+h,r.nextBowlerIndex=o?.bowlingOrder?.length&&m?(o.bowlingOrder.indexOf(m)+1+o.bowlingOrder.length)%o.bowlingOrder.length:0,r.pendingChoices={batting:null,bowling:null},o&&(o.currentBowlerId=m)}),n.updatedAt=t?.updatedAt??Date.now(),n}function xn(e){let t=a(e);return Object.values(t.teams).forEach(e=>{e.playerIds.forEach(t=>{e.stats[t]={runs:0,ballsFaced:0,wicketsTaken:0,ballsBowled:0,dismissals:0}})}),t.ballEvents.forEach(e=>{let n=t.teams[e.battingTeamId],r=t.teams[e.bowlingTeamId],i=e.strikerId,a=e.bowlerId;i&&n?.stats[i]&&(n.stats[i].ballsFaced+=1,n.stats[i].runs+=e.runs,e.resultType===`wicket`&&(n.stats[i].dismissals+=1)),a&&r?.stats[a]&&(r.stats[a].ballsBowled+=1,e.resultType===`wicket`&&(r.stats[a].wicketsTaken+=1))}),t}function Sn(e,t){let n=a(e),r=Number.isInteger(Number(t?.inningsIndex))&&Number(t?.inningsIndex)>=0?Number(t.inningsIndex):n.currentInningsIndex,i=n.innings[r],o=_n(t),s=Number(t?.score),c=Number(t?.wickets),l=vn(t);return i&&(Number.isFinite(s)&&(i.scoreboard.runs=s),Number.isFinite(c)&&(i.scoreboard.wickets=c),Number.isInteger(o)&&(i.scoreboard.legalBalls=o,i.scoreboard.overs=Math.floor(o/6),i.scoreboard.ballsLeft=Math.max(n.settings.overs*6-o,0)),i.scoreboard.target=t?.target==null?i.scoreboard.target:Number(t.target),i.scoreboard.requiredRuns=i.scoreboard.target==null?null:Math.max(i.scoreboard.target-i.scoreboard.runs,0),i.scoreboard.ballsInOver=Number.isInteger(Number(t?.ball))?Number(t.ball)%6:i.scoreboard.ballsInOver,i.strikerId=t?.strikerId??i.strikerId,i.nonStrikerId=t?.nonStrikerId??i.nonStrikerId,i.currentBowlerId=t?.currentBowlerId??i.currentBowlerId,i.pendingChoices={batting:null,bowling:null}),n.revealState={...n.revealState,status:`revealing`,waitingFor:null,lockedSides:[],revealAt:null,lastBall:l},n.ballEvents.some(e=>e.timestamp===l.timestamp)||n.ballEvents.push(l),n.updatedAt=Date.now(),n}var Cn=class{constructor({store:e,socket:t}){this.store=e,this.socket=t,this.timers={revealReset:null,inningsBreak:null,revealLock:null,aiBatting:null,aiBowling:null,aiDecision:null},this.unsubscribers=[this.socket.on(`game_started`,e=>this.handleGameStarted(e)),this.socket.on(`next_turn`,e=>this.handleNextTurn(e)),this.socket.on(`ball_result`,e=>this.handleBallResult(e)),this.socket.on(`match_end`,e=>this.handleMatchEnd(e)),this.socket.on(`game_error`,e=>this.handleGameError(e)),this.socket.on(`match:playerLeft`,e=>this.handlePlayerLeft(e)),this.socket.on(`connection:status`,e=>this.applyConnection(e))]}destroy(){this.clearTimers(),this.unsubscribers.forEach(e=>e())}clearTimer(e){let t=this.timers[e];t&&(window.clearTimeout(t),this.timers[e]=null)}clearTimers(e=Object.keys(this.timers)){e.forEach(e=>this.clearTimer(e))}setBanner(e){this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:e}})}pushToast(e,t=`info`){this.store.dispatch({type:G.PUSH_TOAST,payload:{id:q(`toast`),message:e,tone:t}})}queueOfflineAiSelections(){let e=this.store.getState(),t=e.room,n=e.match;if(!t||!n||!$(t)||e.ui.isPaused||n.phase!==L.LIVE){this.clearTimers([`aiBatting`,`aiBowling`,`revealLock`]);return}let r=n.innings[n.currentInningsIndex],i=!r.pendingChoices.batting&&fn(t,r.strikerId),a=!r.pendingChoices.bowling&&fn(t,r.currentBowlerId);i&&!this.timers.aiBatting&&(this.timers.aiBatting=window.setTimeout(()=>{this.timers.aiBatting=null,this.applyOfflineAiSelection(`batting`)},sn())),i||this.clearTimer(`aiBatting`),a&&!this.timers.aiBowling&&(this.timers.aiBowling=window.setTimeout(()=>{this.timers.aiBowling=null,this.applyOfflineAiSelection(`bowling`)},sn())),a||this.clearTimer(`aiBowling`),r.pendingChoices.batting&&r.pendingChoices.bowling&&this.lockOfflineReveal()}applyOfflineAiSelection(e){let t=this.store.getState(),n=t.room,r=t.match;if(!n||!r||!$(n)||r.phase!==L.LIVE||t.ui.isPaused)return;let i=r.innings[r.currentInningsIndex],a=e===`batting`?i.strikerId:i.currentBowlerId;if(!fn(n,a)||i.pendingChoices[e])return;let o=Kt(r,e,{controller:`ai`,playerId:a,teamId:e===`batting`?i.battingTeamId:i.bowlingTeamId,value:un(r,e),lockedAt:Date.now()});this.store.dispatch({type:G.SET_MATCH,payload:o}),this.setBanner(o.revealState.status===`locked`?`Both picks locked. Revealing together...`:e===`batting`?`AI batter locked in.`:`AI bowler locked in.`),this.queueOfflineAiSelections()}lockOfflineReveal(){let e=this.store.getState(),t=e.room,n=e.match;if(!t||!n||!$(t)||n.phase!==L.LIVE)return;let r=n.innings[n.currentInningsIndex];if(!r.pendingChoices.batting||!r.pendingChoices.bowling)return;this.clearTimers([`aiBatting`,`aiBowling`]);let i=n.revealState.status===`locked`?n:qt(n,{status:`locked`,waitingFor:null,revealAt:Date.now()+U.REVEAL_LOCK});this.store.dispatch({type:G.SET_MATCH,payload:i}),this.setBanner(`Both picks locked. Revealing together...`),this.clearTimer(`revealLock`),this.timers.revealLock=window.setTimeout(()=>{this.timers.revealLock=null,this.resolveOfflineBall()},U.REVEAL_LOCK)}resolveOfflineBall(){let e=this.store.getState(),t=e.room,n=e.match;if(!t||!n||!$(t)||n.phase!==L.LIVE)return;let r=n.innings[n.currentInningsIndex],i=r.pendingChoices.batting?.value??null,a=r.pendingChoices.bowling?.value??null;if(!X(i,n.settings.allowedNumbers)||!X(a,n.settings.allowedNumbers))return;let o=qt(Xt(n,i,a),{status:`revealing`,waitingFor:null,revealAt:null}),s=o.revealState.lastBall?.timestamp??Date.now();this.store.dispatch({type:G.SET_MATCH,payload:o}),this.store.dispatch({type:G.SET_CONNECTION,payload:{status:R.CONNECTED,note:o.revealState.lastBall?.commentary??`Practice ball resolved.`,lastSyncAt:Date.now()}}),this.setBanner(o.revealState.lastBall?.commentary??`Practice ball resolved.`),this.clearTimer(`revealReset`),this.timers.revealReset=window.setTimeout(()=>this.finalizeResolvedBall(s),U.RESULT_DISPLAY)}finalizeOfflineTossDecision(e){let t=this.store.getState().room;!t?.tossResult||!$(t)||(this.clearTimer(`aiDecision`),this.store.dispatch({type:G.SET_ROOM,payload:{...t,status:I.LINEUP,tossResult:{...t.tossResult,decision:e}}}),this.store.dispatch({type:G.NAVIGATE,payload:F.LINEUP}),this.setBanner(`Toss finalized. ${t.teams[t.tossResult.winnerTeamId]?.name??`Winning team`} chose to ${e}.`))}pauseInterruptedMatch(e,t={}){let{resetSelections:n=!1,toast:r=null}=t,i=this.store.getState();this.clearTimers(),i.match&&n&&this.store.dispatch({type:G.SET_MATCH,payload:Yt(i.match)}),this.store.dispatch({type:G.PATCH_UI,payload:{isPaused:!0,connectionBanner:e}}),r&&this.pushToast(r,`warning`)}markPlayerStatus(e,t,n){let r=this.store.getState();r.room&&this.store.dispatch({type:G.SET_ROOM,payload:{...r.room,players:r.room.players.map(r=>r.id===e?{...r,status:t,connectionState:{...r.connectionState,status:t===`away`?R.OFFLINE:R.CONNECTED,note:n,lastSyncAt:Date.now()}}:r)}})}togglePause(){let e=this.store.getState();if(!e.match)return;let t=!e.ui.isPaused;this.store.dispatch({type:G.PATCH_UI,payload:{isPaused:t,connectionBanner:t?`Match paused locally.`:`Match resumed. Lock your number.`}}),!t&&$(e.room)&&this.queueOfflineAiSelections()}simulateReconnect(){if($(this.store.getState().room)){this.setBanner(`Practice mode runs locally. No server reconnect is needed.`);return}this.setBanner(`Trying to reconnect to the realtime backend...`),this.socket.reconnect()}startToss(){let e=this.store.getState(),t=e.room;if(!(!t?.teams.alpha||!t.teams.beta||t.hostId!==e.session.localPlayerId||t.players.length!==t.maxPlayers||t.players.some(e=>e.status!==`ready`)||t.teams.alpha.playerIds.length!==t.settings.playersPerTeam||t.teams.beta.playerIds.length!==t.settings.playersPerTeam)){if($(t)){this.store.dispatch({type:G.SET_ROOM,payload:{...t,status:I.TOSS,tossResult:null}}),this.store.dispatch({type:G.NAVIGATE,payload:F.TOSS}),this.setBanner(`Practice toss ready. Call heads or tails.`);return}this.socket.emit(`start_toss`,{},e=>{e?.ok||this.handleGameError({message:e?.error??`Unable to start the toss.`})})}}runToss(e){let t=this.store.getState().room;if(!(!t?.teams.alpha||!t.teams.beta||t.status!==I.TOSS)){if($(t)){let n=Math.random()>.5?`heads`:`tails`,r=n===e?z.ALPHA:z.BETA,i={...t,tossResult:{call:e,coinFace:n,winnerTeamId:r,decision:null}};this.store.dispatch({type:G.SET_ROOM,payload:i}),i.captains[r]===this.store.getState().session.localPlayerId?this.setBanner(`${i.teams[r]?.name??`Your team`} won the toss. Choose bat or bowl.`):(this.setBanner(`${i.teams[r]?.name??`AI team`} won the toss. AI captain is choosing...`),this.clearTimer(`aiDecision`),this.timers.aiDecision=window.setTimeout(()=>{this.timers.aiDecision=null,this.finalizeOfflineTossDecision(dn({settings:i.settings}))},sn()));return}this.socket.emit(`run_toss`,{call:e},e=>{e?.ok||this.handleGameError({message:e?.error??`Unable to flip the toss.`})})}}chooseTossDecision(e){let t=this.store.getState().room;if(!(!t?.tossResult||t.status!==I.TOSS)){if($(t)){this.finalizeOfflineTossDecision(e);return}this.socket.emit(`choose_toss`,{decision:e},e=>{e?.ok||this.handleGameError({message:e?.error??`Unable to lock the toss decision.`})})}}confirmLineupsAndStart(){let e=this.store.getState(),t=e.room;if(!t?.teams.alpha||!t.teams.beta||!t.tossResult?.decision||t.hostId!==e.session.localPlayerId)return;let n=t.tossResult.decision===`bat`?t.tossResult.winnerTeamId:t.tossResult.winnerTeamId===z.ALPHA?z.BETA:z.ALPHA,r=t.lineups?.[n]?.battingOrder?.[0]??t.teams[n]?.playerIds[0]??null;if(r){if($(t)){this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Game starts within 5 seconds. Preparing practice match...`,isPaused:!1}});let e={...t,status:I.LIVE},n=Gt(e,t.tossResult);this.clearTimers(),this.store.dispatch({type:G.SET_ROOM,payload:e}),this.store.dispatch({type:G.SET_MATCH,payload:n}),this.store.dispatch({type:G.NAVIGATE,payload:F.MATCH}),this.store.dispatch({type:G.SET_CONNECTION,payload:{status:R.CONNECTED,note:`Practice match running locally.`,lastSyncAt:Date.now()}}),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Practice match live. Lock your number when it is your turn.`,isPaused:!1}}),this.queueOfflineAiSelections();return}this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Game starts within 5 seconds. Preparing realtime match...`,isPaused:!1}}),this.socket.emit(`start_match`,{battingFirstPlayerId:r,tossWinnerId:t.teams[t.tossResult.winnerTeamId]?.captainId??null,decision:t.tossResult.decision,teams:mn(t),settings:{overs:t.settings.overs,matchMode:t.settings.matchMode,bowlingMode:t.settings.bowlingMode,numberSetMode:t.settings.numberSetMode,numberSetPreset:t.settings.numberSetPreset,numberSetLabel:t.settings.numberSetLabel,allowedNumbers:t.settings.allowedNumbers,numberRangeMin:t.settings.numberRangeMin,numberRangeMax:t.settings.numberRangeMax,customNumbersText:t.settings.customNumbersText}},e=>{e?.ok||this.handleGameError({message:e?.error??`Unable to start the match.`})})}}submitSelection(e,t,n={}){let r=this.store.getState(),i=r.match,a=r.room,o=r.session.localPlayerId,s=i?.settings.allowedNumbers??S({},{}).settings.allowedNumbers;if(!i||!a||!o||r.ui.isPaused||i.phase!==L.LIVE||!X(t,s)||i.revealState.status===`locked`||i.revealState.status===`revealing`){i&&!X(t,s)&&this.handleGameError({message:`Choose one of: ${s.join(`, `)}.`});return}let c=i.innings[i.currentInningsIndex];if((c.strikerId===o?`batting`:c.currentBowlerId===o?`bowling`:null)!==e||c.pendingChoices[e]?.value!=null)return;if(n.previewOnly){this.store.dispatch({type:G.PATCH_UI,payload:{selectedPicks:{...r.ui.selectedPicks??{batting:null,bowling:null},[e]:t},selectedNumberSide:e}}),this.setBanner(`Selected ${t}. Lock your pick when ready.`);return}let l=e===`batting`?c.battingTeamId:c.bowlingTeamId,u=Kt(i,e,{controller:`local`,playerId:e===`batting`?c.strikerId:c.currentBowlerId,teamId:l,value:t,lockedAt:Date.now()}),d=u.revealState.cycleId;if(this.store.dispatch({type:G.SET_MATCH,payload:u}),this.store.dispatch({type:G.PATCH_UI,payload:{selectedPicks:{...r.ui.selectedPicks??{batting:null,bowling:null},[e]:null},selectedNumberSide:null}}),this.setBanner(`Selection locked. Waiting for opponent.`),$(a)){this.queueOfflineAiSelections(),u.innings[u.currentInningsIndex].pendingChoices.batting&&u.innings[u.currentInningsIndex].pendingChoices.bowling&&this.lockOfflineReveal();return}this.socket.emit(`select_number`,{number:t},t=>{let n=this.store.getState(),r=n.match;if(!(!r||r.revealState.cycleId!==d)&&r.revealState.status!==`revealing`){if(!t?.ok){this.store.dispatch({type:G.SET_MATCH,payload:Yt(r)}),this.store.dispatch({type:G.PATCH_UI,payload:{selectedPicks:{...n.ui.selectedPicks??{batting:null,bowling:null},[e]:null}}}),this.handleGameError({message:t?.error??`Unable to submit selection.`});return}t.waiting===!1&&(this.store.dispatch({type:G.SET_MATCH,payload:qt(r,{status:`locked`,waitingFor:null,revealAt:Date.now()+U.REVEAL_LOCK})}),this.setBanner(`Both picks locked. Revealing together...`))}})}reorderBatters(e,t){let n=this.store.getState();!n.match||n.ui.isPaused||this.store.dispatch({type:G.SET_MATCH,payload:Zt(n.match,e,t)})}reorderBowlers(e,t){let n=this.store.getState();!n.match||n.ui.isPaused||this.store.dispatch({type:G.SET_MATCH,payload:$t(n.match,e,t)})}changeBowler(e,t){let n=this.store.getState();!n.match||n.ui.isPaused||this.store.dispatch({type:G.SET_MATCH,payload:Qt(n.match,e,t)})}startRematch(){let e=this.store.getState().room;e&&(this.clearTimers(),this.store.dispatch({type:G.SET_MATCH,payload:null}),this.store.dispatch({type:G.SET_ROOM,payload:{...e,status:I.TOSS,tossResult:null,teams:{alpha:e.teams.alpha?{...e.teams.alpha,lockedLineup:!1}:null,beta:e.teams.beta?{...e.teams.beta,lockedLineup:!1}:null},lineups:{alpha:e.lineups?.alpha?{...e.lineups.alpha,locked:!1}:null,beta:e.lineups?.beta?{...e.lineups.beta,locked:!1}:null}}}),this.store.dispatch({type:G.PATCH_UI,payload:{isPaused:!1,connectionBanner:`Rematch ready. Toss again to start fresh.`}}),this.store.dispatch({type:G.NAVIGATE,payload:F.TOSS}))}handleGameStarted(e){let t=this.store.getState().room;if(!t?.teams.alpha||!t.teams.beta)return;let n=e?.tossResult?.decision&&e?.tossResult?.winnerTeamId?e.tossResult:t.tossResult?.decision&&t.tossResult.winnerTeamId?t.tossResult:pn(t,e?.innings?.[0]?.battingTeamId??null),r=e?.teams?{alpha:t.teams.alpha?{...t.teams.alpha,playerIds:[...e.teams.alpha.playerIds],captainId:e.teams.alpha.captainId,battingOrder:[...e.teams.alpha.battingOrder],bowlingOrder:[...e.teams.alpha.bowlingOrder],currentBowlerId:e.teams.alpha.bowlingOrder[0]??t.teams.alpha.currentBowlerId,lockedLineup:!0}:t.teams.alpha,beta:t.teams.beta?{...t.teams.beta,playerIds:[...e.teams.beta.playerIds],captainId:e.teams.beta.captainId,battingOrder:[...e.teams.beta.battingOrder],bowlingOrder:[...e.teams.beta.bowlingOrder],currentBowlerId:e.teams.beta.bowlingOrder[0]??t.teams.beta.currentBowlerId,lockedLineup:!0}:t.teams.beta}:t.teams,i=S(e?.settings??{},t.settings),a={...t,status:I.LIVE,tossResult:n,settings:{...t.settings,overs:e?.settings?.overs??t.settings.overs,matchMode:e?.settings?.matchMode??t.settings.matchMode,bowlingMode:e?.settings?.bowlingMode??t.settings.bowlingMode,...i.ok?i.settings:S({},t.settings).settings},teams:r,lineups:{alpha:r.alpha?{teamId:r.alpha.id,battingOrder:[...r.alpha.battingOrder],bowlingOrder:[...r.alpha.bowlingOrder],locked:!0}:null,beta:r.beta?{teamId:r.beta.id,battingOrder:[...r.beta.battingOrder],bowlingOrder:[...r.beta.bowlingOrder],locked:!0}:null}},o=xn(bn(Gt(a,n),e));this.store.dispatch({type:G.SET_ROOM,payload:a}),this.store.dispatch({type:G.SET_MATCH,payload:o}),this.store.dispatch({type:G.NAVIGATE,payload:F.MATCH}),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:e?.ballHistory?.length||e?.innings?.some(e=>Number(e?.legalBalls)>0)?`Realtime match restored. Lock your number when your turn is active.`:`Realtime match live. Lock your number.`,isPaused:!1}})}handleNextTurn(e){let t=this.store.getState(),n=t.match?gn(t.match,e):null,r=t.match?.revealState.status===`revealing`,i=!!e?.waitingFor?.length,a=r?R.CONNECTED:i?R.WAITING:R.CONNECTED,o=r?t.ui.connectionBanner||`Reveal in progress.`:e?.message??`Realtime turn synced.`;this.store.dispatch({type:G.SET_CONNECTION,payload:{status:a,note:o,lastSyncAt:Date.now()}}),n&&this.store.dispatch({type:G.SET_MATCH,payload:n}),e?.timeout&&!r&&this.pushToast(`Still waiting for the missing input.`,`warning`),r||this.setBanner(e?.message??`Realtime turn synced.`),t.room&&this.store.dispatch({type:G.SET_ROOM,payload:{...t.room,status:t.room.status===I.COMPLETED?I.COMPLETED:I.LIVE}})}handleBallResult(e){let t=this.store.getState().match;if(!t||t.phase!==L.LIVE)return;let n=_n(e),r=t.innings[t.currentInningsIndex];if(Number.isInteger(n)&&r.scoreboard.legalBalls>n)return;let i=Number.isInteger(n)&&r.scoreboard.legalBalls>=n?Sn(t,e):qt(Xt(t,e.battingNumber,e.bowlingNumber),{status:`revealing`,waitingFor:null,revealAt:null});if(i.revealState.lastBall&&(i.revealState.lastBall.timestamp=e?.timestamp??i.revealState.lastBall.timestamp,i.revealState.lastBall.commentary=e?.commentary??i.revealState.lastBall.commentary),i.ballEvents.length){let t=i.ballEvents[i.ballEvents.length-1];t.timestamp=e?.timestamp??t.timestamp,t.commentary=e?.commentary??t.commentary}let a=e?.timestamp??i.revealState.lastBall?.timestamp??Date.now();this.store.dispatch({type:G.SET_MATCH,payload:i}),this.store.dispatch({type:G.SET_CONNECTION,payload:{status:R.CONNECTED,note:e?.commentary??i.revealState.lastBall?.commentary??`Ball synced.`,lastSyncAt:Date.now()}}),this.setBanner(e?.commentary??i.revealState.lastBall?.commentary??`Ball synced.`),this.clearTimer(`revealReset`),this.timers.revealReset=window.setTimeout(()=>this.finalizeResolvedBall(a),U.RESULT_DISPLAY)}finalizeResolvedBall(e){let t=this.store.getState(),n=t.match,r=t.room;if(!(!n||n.revealState.lastBall?.timestamp!==e)){if(n.phase===L.BREAK){this.clearTimer(`inningsBreak`),this.timers.inningsBreak=window.setTimeout(()=>{let e=this.store.getState();!e.match||e.match.phase!==L.BREAK||(this.store.dispatch({type:G.SET_MATCH,payload:Wt(e.match)}),this.setBanner(`Innings switched. Lock fresh picks.`),$(e.room)&&this.queueOfflineAiSelections())},U.INNINGS_BREAK);return}if(n.phase===L.COMPLETE&&n.result&&r){this.store.dispatch({type:G.SAVE_HISTORY_ENTRY,payload:en(n,r.code)}),this.store.dispatch({type:G.SET_ROOM,payload:{...r,status:I.COMPLETED}}),this.store.dispatch({type:G.NAVIGATE,payload:F.RESULT}),this.setBanner(n.result.marginText);return}this.store.dispatch({type:G.SET_MATCH,payload:Jt(n)}),this.setBanner(`Next ball ready. Make your picks.`),$(r)&&this.queueOfflineAiSelections()}}handleMatchEnd(e){let t=this.store.getState();if(e?.aborted){this.pauseInterruptedMatch(e.reason??`Match ended early.`,{resetSelections:!0,toast:e.reason??`Match ended early.`});return}t.room&&this.store.dispatch({type:G.SET_ROOM,payload:{...t.room,status:I.COMPLETED}}),e?.result?.marginText&&this.setBanner(e.result.marginText)}handlePlayerLeft(e){let t=this.store.getState();if(!t.room||!t.match||$(t.room)||e.roomId&&e.roomId!==t.room.code)return;let n=t.room.players.find(t=>t.id===e.playerId)?.name??`Opponent`;e.playerId&&this.markPlayerStatus(e.playerId,`away`,e.note??`Left the room`),this.pauseInterruptedMatch(`${n} left mid-match. Match paused.`,{resetSelections:!0,toast:`${n} disconnected during the round.`})}handleGameError(e){let t=e?.message??`Realtime game action failed.`;this.pushToast(t,`warning`),this.setBanner(t)}applyConnection(e){let t=this.store.getState();if($(t.room)){this.store.dispatch({type:G.SET_CONNECTION,payload:{status:R.CONNECTED,note:`Practice mode active locally.`,lastSyncAt:Date.now()}});return}if(this.store.dispatch({type:G.SET_CONNECTION,payload:{status:e.status,note:e.note,lastSyncAt:Date.now()}}),e.status===R.RECONNECTING&&t.match){this.pauseInterruptedMatch(`Realtime link unstable. Waiting to reconnect.`,{resetSelections:!0});return}if(e.status===R.OFFLINE&&t.match){this.pauseInterruptedMatch(e.note||`Connection lost.`,{resetSelections:!0});return}e.status===R.CONNECTED&&!t.ui.isPaused&&this.setBanner(e.note||(t.match?`Realtime match synced.`:`Connected.`))}},wn=null,Tn=24,En=1e4;function Dn(){let e=window.MH_HANDREX_SOCKET_URL;if(typeof e==`string`&&e.trim())return e.trim().replace(/\/$/,``);if(window.location.protocol.startsWith(`http`)){let e=new URLSearchParams(window.location.search);return e.get(`gamehubShell`)===`premium`||e.get(`gamehubEmbedded`)===`1`?`${window.location.origin}/premium-handcricket`:window.location.origin}return`http://localhost:4000`}function On(e){try{return new URL(e,window.location.href).origin}catch{return window.location.protocol.startsWith(`http`)?window.location.origin:`http://localhost:4000`}}function kn(e){return typeof window.io==`function`?Promise.resolve(window.io):wn||(wn=new Promise((t,n)=>{let r=document.createElement(`script`);r.src=`${On(e)}/socket.io/socket.io.js`,r.async=!0,r.onload=()=>{if(typeof window.io==`function`){t(window.io);return}n(Error(`Socket.io client loaded but window.io was unavailable.`))},r.onerror=()=>{n(Error(`Failed to load Socket.io client from backend.`))},document.head.append(r)}),wn)}function An(e){return e.startsWith(`connection:`)||e.startsWith(`socket:`)||e.startsWith(`match:`)}var jn=class{constructor(){this.url=Dn(),this.socket=null,this.listeners=new Map,this.serverListeners=new Map,this.timers=new Set,this.pendingEmits=[],this.initializing=null,this.initialize()}async initialize(){return this.socket?this.socket:this.initializing?this.initializing:(this.emitLocal(`connection:status`,{status:R.RECONNECTING,note:`Connecting to realtime backend...`}),this.initializing=kn(this.url).then(e=>(this.socket=e(this.url,{autoConnect:!0,reconnection:!0,reconnectionDelay:500,reconnectionDelayMax:4e3,randomizationFactor:.5,timeout:8e3,path:`/socket.io`,transports:[`websocket`,`polling`]}),this.registerCoreHandlers(),this.bindQueuedServerListeners(),this.socket.connected&&this.flushPendingEmits(),this.socket)).catch(e=>(this.pendingEmits.splice(0).forEach(t=>{t.ack?.({ok:!1,error:e instanceof Error?e.message:`Unable to connect to the realtime backend.`})}),this.emitLocal(`connection:status`,{status:R.OFFLINE,note:e instanceof Error?e.message:`Unable to connect to the realtime backend.`}),null)).finally(()=>{this.initializing=null}),this.initializing)}registerCoreHandlers(){this.socket&&(this.socket.on(`connect`,()=>{this.emitLocal(`connection:status`,{status:R.CONNECTED,note:`Connected to realtime backend`}),this.flushPendingEmits()}),this.socket.on(`connect_error`,()=>{this.emitLocal(`connection:status`,{status:R.RECONNECTING,note:`Trying to reach realtime backend...`})}),this.socket.on(`disconnect`,e=>{this.emitLocal(`connection:status`,{status:R.OFFLINE,note:`Disconnected: ${e}`})}),this.socket.on(`connected`,e=>{this.emitLocal(`socket:connected`,e)}),this.socket.on(`player_left`,e=>{this.emitLocal(`match:playerLeft`,{roomId:e?.roomId??null,playerId:e?.playerId??null,note:e?.reason??`Player left the room.`})}))}bindQueuedServerListeners(){this.socket&&this.serverListeners.forEach((e,t)=>{e.forEach(e=>{this.socket.on(t,e)})})}flushPendingEmits(){!this.socket||!this.pendingEmits.length||this.pendingEmits.splice(0).forEach(e=>{this.emitNow(e.eventName,e.payload,e.ack)})}queueEmit(e){this.pendingEmits.length>=Tn&&this.pendingEmits.shift()?.ack?.({ok:!1,error:`Realtime connection is busy. Please try that action again.`}),this.pendingEmits.push(e)}emitNow(e,t,n){if(!this.socket){n?.({ok:!1,error:`Realtime backend is not ready yet.`});return}if(typeof n!=`function`){this.socket.emit(e,t);return}let r=!1,i=window.setTimeout(()=>{r||(r=!0,n({ok:!1,error:`Realtime action timed out. Please try again.`}))},En);this.socket.emit(e,t,e=>{r||(r=!0,window.clearTimeout(i),n(e))})}on(e,t){let n=An(e)?this.listeners:this.serverListeners,r=n.get(e)??new Set;return r.add(t),n.set(e,r),!An(e)&&this.socket&&this.socket.on(e,t),()=>{r.delete(t),!An(e)&&this.socket&&this.socket.off(e,t)}}emitLocal(e,t){let n=this.listeners.get(e);n&&n.forEach(e=>e(t))}emit(e,t,n){if(!this.socket||!this.socket.connected){this.queueEmit({eventName:e,payload:t,ack:n}),this.socket?this.socket.connect():this.initialize();return}this.emitNow(e,t,n)}broadcast(e,t,n=0,r){let i=window.setTimeout(()=>{this.timers.delete(i),this.emit(e,t,r)},n);return this.timers.add(i),i}clear(){this.timers.forEach(e=>window.clearTimeout(e)),this.timers.clear()}reconnect(){if(!this.socket){this.initialize();return}if(this.socket.connected){this.emitLocal(`connection:status`,{status:R.CONNECTED,note:`Realtime link already active`});return}this.emitLocal(`connection:status`,{status:R.RECONNECTING,note:`Reconnecting to realtime backend...`}),this.socket.connect()}disconnect(){this.clear(),this.pendingEmits.splice(0).forEach(e=>{e.ack?.({ok:!1,error:`Realtime connection closed before the action was sent.`})}),this.socket?.disconnect()}get id(){return this.socket?.id??null}};function Mn(e){return RegExp(`^[ABCDEFGHJKLMNPQRSTUVWXYZ23456789]{6}$`).test(e)}function Nn(e,t=V){let n=S(e,t);return{...V,...t,...e,...n.ok?n.settings:S({},V).settings}}function Pn(e){let t=document.createElement(`textarea`);t.value=e,t.setAttribute(`readonly`,`true`),t.style.position=`fixed`,t.style.opacity=`0`,t.style.pointerEvents=`none`,document.body.append(t),t.select(),t.setSelectionRange(0,t.value.length);let n=!1;try{n=!!document.execCommand?.(`copy`)}catch{n=!1}return t.remove(),n}function Fn(e,t,n,r,i){return{id:e,name:t,captainId:n,playerIds:[...r],battingOrder:[...r],bowlingOrder:[...r],currentBowlerId:r[0]??null,lockedLineup:i,stats:{}}}function In(e,t){let n=t?.battingOrder&&Dt(t.battingOrder,e.playerIds)?[...t.battingOrder]:[...e.battingOrder],r=t?.bowlingOrder&&Dt(t.bowlingOrder,e.playerIds)?[...t.bowlingOrder]:[...e.bowlingOrder];return{teamId:e.id,battingOrder:n,bowlingOrder:r,locked:e.lockedLineup}}function Ln(e,t,n=null){return e&&t.includes(e)&&e!==n?e:t.find(e=>e!==n)??null}function Rn(e,t,n){let r=new Set(e.map(e=>e.id)),i=new Set,a={alpha:[],beta:[]};return[z.ALPHA,z.BETA].forEach(e=>{(Array.isArray(n?.[e])?n[e]:[]).forEach(n=>{!r.has(n)||i.has(n)||a[e].length>=t||(a[e].push(n),i.add(n))})}),a}function zn(e,t){return e.alpha.length===t&&e.beta.length===t}function Bn(e,t){return e.alpha.includes(t)?z.ALPHA:e.beta.includes(t)?z.BETA:null}function Vn(e,t,n,r){if(Bn(e,t)===n)return{ok:!0,selections:e};if(n&&e[n].length>=r)return{ok:!1,error:`${n===z.ALPHA?`Alpha`:`Beta`} team is already full.`};let i={alpha:e.alpha.filter(e=>e!==t),beta:e.beta.filter(e=>e!==t)};return n&&i[n].push(t),{ok:!0,selections:i}}function Hn(e,t,n,r,i){let a=!!(r?.lineups?.alpha?.locked||r?.lineups?.beta?.locked||r?.status===I.LINEUP||r?.status===I.LIVE||r?.status===I.COMPLETED),o=Rn(e,n,i??r?.teamSelections),s=o.alpha,c=o.beta,l=Ln(t.alpha,s)??s[0]??null,u=Ln(t.beta,c,l)??c[0]??null,d=s.length?Fn(z.ALPHA,r?.teams?.alpha?.name??r?.teamNames?.alpha??`Alpha XI`,l??s[0],s,a):null,f=c.length?Fn(z.BETA,r?.teams?.beta?.name??r?.teamNames?.beta??`Beta XI`,u??c[0],c,a):null;return{alpha:d,beta:f,lineups:{alpha:d?In(d,r?.lineups?.alpha):null,beta:f?In(f,r?.lineups?.beta):null},captains:{alpha:d?.captainId??null,beta:f?.captainId??null},selections:o}}function Un(e,t){return e?{...e,teamNames:{alpha:t?.alpha?.trim()||e.teamNames?.alpha||e.teams?.alpha?.name||`Alpha XI`,beta:t?.beta?.trim()||e.teamNames?.beta||e.teams?.beta?.name||`Beta XI`},teams:{alpha:e.teams?.alpha?{...e.teams.alpha,name:t?.alpha?.trim()||e.teams.alpha.name||`Alpha XI`}:e.teams?.alpha??null,beta:e.teams?.beta?{...e.teams.beta,name:t?.beta?.trim()||e.teams.beta.name||`Beta XI`}:e.teams?.beta??null}}:null}function Wn(e,t,n){let r=e.session.localPlayerId??q(`local`),i=Qe(),a=an(),o=rn(n*2-1,t).map((e,t)=>({...e,role:t===0?`captain`:`player`,status:`ready`,connectionState:{...e.connectionState,note:`Practice AI ready`}})),s={alpha:r,beta:o[0]?.id??null},c=[{id:r,name:t,isHost:!0,isLocal:!0,isMock:!1,role:`captain`,status:`unready`,avatarSeed:t.toLowerCase().replace(/\s+/g,`-`),connectionState:{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:0,note:`Practice player`}},...o],l={alpha:[r],beta:o[0]?.id?[o[0].id]:[]};o.filter(e=>!l.beta.includes(e.id)).forEach(e=>{if(l.alpha.length<n){l.alpha.push(e.id);return}l.beta.length<n&&l.beta.push(e.id)});let u={id:q(`practice-room`),code:i,source:`offline`,teamNames:a,hostId:r,maxPlayers:n*2,playerIds:c.map(e=>e.id),players:c,status:I.LOBBY,connectionState:{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:0,note:`Offline practice room with AI players.`},settings:{...Nn(V),playersPerTeam:n,controlMode:_e.LOCAL_VS_AI},captains:s,teamSelections:l,teams:{alpha:null,beta:null},lineups:{alpha:null,beta:null},draftState:{status:`complete`,availablePlayerIds:[],currentPickIndex:0,pickSequence:[],picks:[]},tossResult:null,createdAt:Date.now()},d=Hn(c,s,n,u,l),f={alpha:d.alpha?{...d.alpha,name:a.alpha}:null,beta:d.beta?{...d.beta,name:a.beta}:null};return{...u,captains:d.captains,teamSelections:d.selections,teams:f,lineups:{alpha:f.alpha?In(f.alpha,d.lineups.alpha):null,beta:f.beta?In(f.beta,d.lineups.beta):null},players:c.map(e=>({...e,role:e.id===d.captains.alpha||e.id===d.captains.beta?`captain`:`player`}))}}function Gn(e,t,n,r){return e===`toss`?I.TOSS:e===`lineup`?I.LINEUP:e===`live`?I.LIVE:e===`completed`?I.COMPLETED:e===`draft`?I.DRAFT:I.LOBBY}function Kn(e,t,n){return e===`live`?{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:28,note:`Realtime match is live.`}:e===`completed`?{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:28,note:`Match completed.`}:e===`toss`?{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:28,note:`Toss live for the captains.`}:e===`lineup`?{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:28,note:`Lineups are being finalized.`}:t<n||e===`waiting`?{status:R.WAITING,lastSyncAt:Date.now(),latencyMs:28,note:`Waiting for ${Math.max(n-t,0)} more player${n-t===1?``:`s`}.`}:{status:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:28,note:`Room synced. Ready states are live.`}}function qn(e,t){let n=Un(t.room,e.teamNames)??{teamNames:e.teamNames??{alpha:`Alpha XI`,beta:`Beta XI`},teams:{alpha:null,beta:null},lineups:t.room?.lineups,status:t.room?.status,settings:t.room?.settings??V,captains:t.room?.captains??{alpha:null,beta:null},teamSelections:t.room?.teamSelections??{alpha:[],beta:[]}},r=t.session.localPlayerId,i=e.players.map(e=>e.id),a=Math.max(1,Number(e.teamSize)||e.settings?.playersPerTeam||n?.settings?.playersPerTeam||V.playersPerTeam),o=Math.max(2,Number(e.maxPlayers)||a*2),s=e.players.map(t=>({id:t.id,name:t.name,isHost:t.id===e.hostId,isLocal:t.id===r,isMock:!1,role:`player`,status:t.connected===!1?`away`:t.ready?`ready`:`unready`,avatarSeed:t.name.toLowerCase().replace(/\s+/g,`-`),connectionState:{status:t.connected===!1?R.OFFLINE:R.CONNECTED,lastSyncAt:Date.now(),latencyMs:28,note:t.connected===!1?`Disconnected from room`:t.ready?`Ready in lobby`:`Connected`}})),c=Hn(s,{alpha:e.captains?.alpha??n?.captains?.alpha??null,beta:e.captains?.beta??n?.captains?.beta??null},a,n,e.teamSelections??n?.teamSelections),l=s.map(e=>({...e,role:e.id===c.captains.alpha||e.id===c.captains.beta?`captain`:`player`})),u=Gn(e.status,n?.status,s.length,o),d=s.length>=o,f=zn(c.selections,a);return{id:e.roomId,code:e.roomId,source:`realtime`,teamNames:{alpha:c.alpha?.name??e.teamNames?.alpha??`Alpha XI`,beta:c.beta?.name??e.teamNames?.beta??`Beta XI`},hostId:e.hostId,maxPlayers:o,playerIds:i,players:l,status:u,connectionState:Kn(e.status,s.length,o),settings:{...Nn(e.settings??{},n?.settings??V),playersPerTeam:a,controlMode:_e.LOCAL_VS_AI},captains:{alpha:c.captains.alpha,beta:c.captains.beta},teamSelections:c.selections,teams:{alpha:c.alpha,beta:c.beta},lineups:c.lineups,draftState:{status:d&&f?`complete`:`pending`,availablePlayerIds:[],currentPickIndex:0,pickSequence:[],picks:[]},tossResult:e.tossResult??n?.tossResult??null,createdAt:e.createdAt??n?.createdAt??Date.now()}}var Jn=class{constructor({store:e,socket:t}){this.store=e,this.socket=t,this.pendingRoomAction=null,this.resumeInFlight=!1,this.unsubscribers=[this.socket.on(`socket:connected`,e=>this.handleSocketConnected(e)),this.socket.on(`room_created`,e=>this.handleRoomCreated(e)),this.socket.on(`room_update`,e=>this.handleRoomUpdate(e)),this.socket.on(`room_kicked`,e=>this.handleRoomKicked(e)),this.socket.on(`room_closed`,e=>this.handleRoomClosed(e)),this.socket.on(`room_error`,e=>this.handleRoomError(e)),this.socket.on(`connection:status`,e=>this.setConnection(e))]}destroy(){this.unsubscribers.forEach(e=>e())}clearPendingRoomAction(){this.pendingRoomAction=null}beginRoomAction(e){return this.pendingRoomAction||this.resumeInFlight?(this.pushToast(`Please wait for the current room request to finish.`,`info`),!1):(this.pendingRoomAction=e,!0)}clearActiveRoomSession(){let e=this.store.getState();this.store.dispatch({type:G.UPDATE_SESSION,payload:{...e.session,activeRoomCode:``}})}attemptResume(){let e=this.store.getState(),t=e.session.activeRoomCode?.trim().toUpperCase(),n=e.session.playerKey;!t||!n||this.resumeInFlight||(this.resumeInFlight=!0,this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Rejoining room ${t}...`}}),this.socket.emit(`resume_session`,{roomId:t,playerKey:n,playerName:e.session.profileName},e=>{if(this.resumeInFlight=!1,e?.ok){this.pushToast(`Rejoined room ${t}.`,`success`);return}this.clearActiveRoomSession(),this.handleRoomError({message:e?.error??`Unable to restore the previous room session.`})}))}pushToast(e,t=`info`){this.store.dispatch({type:G.PUSH_TOAST,payload:{id:q(`toast`),message:e,tone:t}})}setConnection(e){let t=this.store.getState();if(t.room?.source===`offline`){this.store.dispatch({type:G.SET_CONNECTION,payload:{status:R.CONNECTED,note:t.room.connectionState.note??`Practice room active locally.`,lastSyncAt:Date.now()}});return}this.store.dispatch({type:G.SET_CONNECTION,payload:{status:e.status,note:e.note,lastSyncAt:Date.now()}}),t.room&&this.store.dispatch({type:G.SET_ROOM,payload:{...t.room,connectionState:{...t.room.connectionState,status:e.status,note:e.note,lastSyncAt:Date.now()}}})}handleSocketConnected(e){e?.socketId&&this.store.getState().room?.source!==`offline`&&(this.store.dispatch({type:G.UPDATE_SESSION,payload:{localPlayerId:e.socketId}}),this.store.getState().session.activeRoomCode&&this.attemptResume())}applyServerRoom(e){let t=this.store.getState(),n=qn(e,t),r=zn(n.teamSelections,n.settings.playersPerTeam),i=null;this.resumeInFlight=!1,this.clearPendingRoomAction(),n.status===I.TOSS&&t.route!==F.MATCH&&t.route!==F.RESULT?i=F.TOSS:n.status===I.LINEUP&&t.route!==F.MATCH&&t.route!==F.RESULT?i=F.LINEUP:n.status===I.LOBBY&&[F.TOSS,F.LINEUP].includes(t.route)?i=n.playerIds.length===n.maxPlayers?F.LOBBY:F.ROOM:n.playerIds.length===n.maxPlayers&&n.status===I.LOBBY&&[F.HOME,F.ROOM,F.TOSS,F.LINEUP].includes(t.route)?i=F.LOBBY:(!t.room||t.route===F.HOME)&&n.status!==I.LIVE&&n.status!==I.COMPLETED&&(i=n.playerIds.length===n.maxPlayers?F.LOBBY:F.ROOM);let a=`Room synced.`;a=n.status===I.TOSS?n.tossResult?n.tossResult.decision?`${n.teams[n.tossResult.winnerTeamId]?.name??`Winning team`} chose to ${n.tossResult.decision}.`:`${n.teams[n.tossResult.winnerTeamId]?.name??`Winning team`} won the toss. Waiting for the decision.`:`Toss live. Alpha captain can call the flip.`:n.status===I.LINEUP?`Toss complete. Captains can finalize batting and bowling orders.`:n.playerIds.length<n.maxPlayers?`Room live. Waiting for ${n.maxPlayers-n.playerIds.length} more player${n.maxPlayers-n.playerIds.length===1?``:`s`} to join.`:r?n.players.every(e=>e.status===`ready`)?`All players ready. Host can continue.`:`Room synced. Ready toggles are live.`:`Choose sides for Alpha and Beta before the host starts the toss.`,this.store.dispatch({type:G.UPDATE_SESSION,payload:{lastRoomCode:n.code,activeRoomCode:n.code}}),this.store.dispatch({type:G.SET_ROOM,payload:n}),t.match||this.store.dispatch({type:G.SET_MATCH,payload:null}),i&&this.store.dispatch({type:G.NAVIGATE,payload:i}),this.store.dispatch({type:G.PATCH_UI,payload:{roomCodeInput:n.code,connectionBanner:a}})}handleRoomCreated(e){this.applyServerRoom(e),this.pushToast(`Room ${e.roomId} created. Share the invite link or room code to join.`,`success`)}handleRoomUpdate(e){this.applyServerRoom(e)}handleRoomKicked(e){let t=e?.roomId??this.store.getState().room?.code??``,n=e?.message??`You were removed from the room by the host.`;this.resumeInFlight=!1,this.clearPendingRoomAction(),this.store.dispatch({type:G.RESET_FLOW}),this.store.dispatch({type:G.PATCH_UI,payload:{roomCodeInput:t,connectionBanner:n}}),this.pushToast(n,`warning`)}handleRoomClosed(e){let t=e?.roomId??this.store.getState().room?.code??``,n=e?.message??`This room was discarded by the host.`;this.resumeInFlight=!1,this.clearPendingRoomAction(),this.store.dispatch({type:G.RESET_FLOW}),this.store.dispatch({type:G.PATCH_UI,payload:{roomCodeInput:t,connectionBanner:n}}),this.pushToast(n,`info`)}handleRoomError(e){this.clearPendingRoomAction();let t=e?.message??`Room action failed.`;this.pushToast(t,`warning`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:t}})}getInviteLink(e=this.store.getState().room?.code??``){if(!e)return``;let t=new URL(window.location.href);return t.search=``,t.hash=``,t.searchParams.set(`room`,e),t.toString()}copyInviteLink(){if(this.store.getState().room?.source===`offline`){this.pushToast(`Invite links are only available for realtime rooms.`,`info`);return}let e=this.getInviteLink();if(!e){this.pushToast(`Create or join a room first to share an invite link.`,`warning`);return}if(navigator.clipboard?.writeText){navigator.clipboard.writeText(e).then(()=>{this.pushToast(`Invite link copied.`,`success`)}).catch(()=>{if(Pn(e)){this.pushToast(`Invite link copied.`,`success`);return}this.pushToast(`Unable to copy automatically. Please copy the room link from the browser address bar.`,`warning`)});return}if(Pn(e)){this.pushToast(`Invite link copied.`,`success`);return}this.pushToast(`Unable to copy automatically. Please copy the room link from the browser address bar.`,`warning`)}createRoom(e={}){let t=(e.playerName||`Captain You`).trim()||`Captain You`,n=Math.max(1,Math.min(11,Number(e.teamSize)||this.store.getState().ui.playersPerTeam||V.playersPerTeam));if(!this.beginRoomAction(`create`))return;let r=this.store.getState();this.store.dispatch({type:G.UPDATE_SESSION,payload:{profileName:t,playerKey:r.session.playerKey}}),this.store.dispatch({type:G.SET_MATCH,payload:null}),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Creating realtime ${n}v${n} room...`,playersPerTeam:n}}),this.socket.emit(`create_room`,{playerName:t,playerKey:r.session.playerKey,teamSize:n,settings:Nn({playersPerTeam:n},V)},e=>{this.clearPendingRoomAction(),!e?.ok&&this.handleRoomError({message:e?.error??`Unable to create room.`})})}joinRoom(e={}){let t=(e.playerName||`Captain You`).trim()||`Captain You`,n=(e.code||``).trim().toUpperCase();if(!Mn(n)){this.handleRoomError({message:`Enter a valid 6-character room code.`});return}if(!this.beginRoomAction(`join`))return;let r=this.store.getState();this.store.dispatch({type:G.UPDATE_SESSION,payload:{profileName:t,playerKey:r.session.playerKey}}),this.store.dispatch({type:G.SET_MATCH,payload:null}),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Joining room ${n}...`}}),this.socket.emit(`join_room`,{roomId:n,playerName:t,playerKey:r.session.playerKey},e=>{if(this.clearPendingRoomAction(),e?.ok){this.pushToast(`Joined room ${n}.`,`success`);return}this.handleRoomError({message:e?.error??`Unable to join room.`})})}startQuickMatch(e={}){this.createRoom({playerName:this.store.getState().session.profileName,teamSize:e.teamSize})}startPracticeMode(){let e=this.store.getState(),t=Number(e.ui.playersPerTeam)||1,n=Math.max(1,Math.min(2,t));t>2&&this.pushToast(`Practice mode currently supports up to 2v2. Switched to 2v2.`,`info`);let r=Wn(e,e.session.profileName,n);this.store.dispatch({type:G.UPDATE_SESSION,payload:{localPlayerId:r.hostId,profileName:e.session.profileName,activeRoomCode:``}}),this.store.dispatch({type:G.SET_MATCH,payload:null}),this.store.dispatch({type:G.SET_ROOM,payload:r}),this.store.dispatch({type:G.SET_CONNECTION,payload:r.connectionState}),this.store.dispatch({type:G.PATCH_UI,payload:{playersPerTeam:n,roomCodeInput:r.code,connectionBanner:`Practice room ready. Review settings, ready up, and start the toss.`,isPaused:!1}}),this.store.dispatch({type:G.NAVIGATE,payload:F.LOBBY}),this.pushToast(`Practice ${n}v${n} room created with AI players.`,`success`)}enterLobby(){this.store.getState().room&&this.store.dispatch({type:G.NAVIGATE,payload:F.LOBBY})}openDraftBoard(){if(this.store.getState().room?.source===`offline`){this.pushToast(`Practice teams are already prepared with AI players.`,`info`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Practice teams are ready. Review lineups and move into the toss.`}});return}this.pushToast(`Players now choose sides directly in the room and lobby before the toss.`,`info`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Set team sides, choose captains from each side, then move into the toss.`}})}pickDraftPlayer(e){}autoDraftTeams(){if(this.store.getState().room?.source===`offline`){this.pushToast(`Practice teams are already filled with AI players.`,`info`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Practice teams are already set. Start the toss when you are ready.`}});return}this.pushToast(`Manual side selection is active. Use the team board to place players into Alpha or Beta.`,`info`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Arrange both sides manually, then review captains and ready states.`}})}updateSettings(e){let t=this.store.getState();if(!t.room||t.room.hostId!==t.session.localPlayerId)return;if(t.match||t.room.status===I.LIVE||t.room.status===I.COMPLETED){this.pushToast(`Room settings are locked after the match starts.`,`warning`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Room settings are locked after the match starts.`,numberSetValidationError:`Room settings are locked after the match starts.`}});return}let n=S(e,t.room.settings);if(!n.ok){this.pushToast(n.error,`warning`),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:n.error,numberSetValidationError:n.error}});return}let r={...t.room.settings,...e,...n.settings,playersPerTeam:t.room.settings.playersPerTeam,controlMode:_e.LOCAL_VS_AI},i=[`numberSetMode`,`numberSetPreset`,`numberRangeMin`,`numberRangeMax`,`customNumbersText`,`allowedNumbers`].some(t=>Object.prototype.hasOwnProperty.call(e,t));this.store.dispatch({type:G.SET_ROOM,payload:{...t.room,settings:r}}),this.store.dispatch({type:G.PATCH_UI,payload:{numberSetDraft:i?null:t.ui.numberSetDraft??null,numberSetValidationError:i?``:t.ui.numberSetValidationError??``,connectionBanner:i?`${r.numberSetLabel} number set applied.`:`Room settings updated.`}}),t.room.source!==`offline`&&this.socket.emit(`update_room_config`,{settings:{matchMode:r.matchMode,bowlingMode:r.bowlingMode,overs:r.overs,numberSetMode:r.numberSetMode,numberSetPreset:r.numberSetPreset,numberSetLabel:r.numberSetLabel,allowedNumbers:r.allowedNumbers,numberRangeMin:r.numberRangeMin,numberRangeMax:r.numberRangeMax,customNumbersText:r.customNumbersText}},e=>{e?.ok||(this.store.dispatch({type:G.PATCH_UI,payload:{numberSetValidationError:e?.error??`Unable to update room settings.`}}),this.handleRoomError({message:e?.error??`Unable to update room settings.`}))})}updateTeamName(e,t){let n=this.store.getState(),r=n.room;if(!r||r.hostId!==n.session.localPlayerId)return;let i=e===`alpha`?`Alpha XI`:`Beta XI`,a=t.trim()||i,o={...r,teamNames:{alpha:e===`alpha`?a:r.teams.alpha?.name??`Alpha XI`,beta:e===`beta`?a:r.teams.beta?.name??`Beta XI`},teams:{...r.teams,[e]:r.teams[e]?{...r.teams[e],name:a}:r.teams[e]}};this.store.dispatch({type:G.SET_ROOM,payload:o}),r.source!==`offline`&&this.socket.emit(`update_room_config`,{teamNames:{alpha:e===`alpha`?a:r.teams.alpha?.name??`Alpha XI`,beta:e===`beta`?a:r.teams.beta?.name??`Beta XI`}},e=>{e?.ok||this.handleRoomError({message:e?.error??`Unable to update team names.`})})}setCaptain(e,t){let n=this.store.getState();if(!n.room||n.room.hostId!==n.session.localPlayerId)return;if(!n.room.teamSelections?.[e]?.includes(t)){this.pushToast(`${e===z.ALPHA?`Alpha`:`Beta`} captain must belong to that side first.`,`info`);return}let r={...n.room.captains,[e]:t};if(r.alpha&&r.alpha===r.beta)return;let i=Hn(n.room.players,r,n.room.settings.playersPerTeam,n.room,n.room.teamSelections);this.store.dispatch({type:G.SET_ROOM,payload:{...n.room,captains:i.captains,teamSelections:i.selections,teams:{alpha:i.alpha,beta:i.beta},lineups:i.lineups,draftState:{...n.room.draftState??{status:`pending`,availablePlayerIds:[],currentPickIndex:0,pickSequence:[],picks:[]},status:n.room.playerIds.length===n.room.maxPlayers&&zn(i.selections,n.room.settings.playersPerTeam)?`complete`:`pending`}}}),n.room.source!==`offline`&&this.socket.emit(`update_room_config`,{captains:i.captains},e=>{e?.ok||this.handleRoomError({message:e?.error??`Unable to update captains.`})})}setPlayerTeam(e,t){let n=this.store.getState(),r=n.room,i=n.session.localPlayerId;if(!r||!e||!i||!(r.hostId===i||e===i))return;let a=t===z.ALPHA||t===z.BETA?t:null;if(r.source===`offline`){let t=Vn(Rn(r.players,r.settings.playersPerTeam,r.teamSelections),e,a,r.settings.playersPerTeam);if(!t.ok){this.pushToast(t.error,`warning`);return}let n=Hn(r.players,r.captains,r.settings.playersPerTeam,r,t.selections);this.store.dispatch({type:G.SET_ROOM,payload:{...r,captains:n.captains,teamSelections:n.selections,teams:{alpha:n.alpha,beta:n.beta},lineups:n.lineups}});return}this.socket.emit(`set_player_team`,{playerId:e,teamId:a},e=>{e?.ok||this.handleRoomError({message:e?.error??`Unable to change team side.`})})}updateTeamLineup(e,t,n){let r=this.store.getState().room,i=r?.teams[e],a=r?.lineups?.[e];if(!r||!i||!a||!Dt(n,i.playerIds))return;let o={...i,battingOrder:t===`batting`?n:i.battingOrder,bowlingOrder:t===`bowling`?n:i.bowlingOrder,currentBowlerId:t===`bowling`?n[0]??i.currentBowlerId:i.currentBowlerId},s={...a,battingOrder:t===`batting`?n:a.battingOrder,bowlingOrder:t===`bowling`?n:a.bowlingOrder};this.store.dispatch({type:G.SET_ROOM,payload:{...r,teams:{...r.teams,[e]:o},lineups:{...r.lineups,[e]:s}}})}lockLineups(){let e=this.store.getState().room;!e?.teams.alpha||!e.teams.beta||!e.lineups?.alpha||!e.lineups.beta||this.store.dispatch({type:G.SET_ROOM,payload:{...e,status:I.LINEUP,teams:{alpha:{...e.teams.alpha,lockedLineup:!0},beta:{...e.teams.beta,lockedLineup:!0}},lineups:{alpha:{...e.lineups.alpha,locked:!0},beta:{...e.lineups.beta,locked:!0}}}})}resetToHome(){this.resumeInFlight=!1,this.clearPendingRoomAction();let e=this.store.getState();e.room?.source===`realtime`&&this.socket.emit(`leave_room`,{},()=>{}),this.store.dispatch({type:G.RESET_FLOW}),e.room?.source===`offline`&&(this.socket.id&&this.store.dispatch({type:G.UPDATE_SESSION,payload:{localPlayerId:this.socket.id}}),this.store.dispatch({type:G.SET_CONNECTION,payload:{status:this.socket.id?R.CONNECTED:R.RECONNECTING,note:this.socket.id?`Connected to realtime backend`:`Connecting to realtime backend`,lastSyncAt:Date.now()}}))}discardRoom(){let e=this.store.getState();if(e.room){if(e.room.source===`offline`){this.resetToHome(),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:`Practice room discarded.`}}),this.pushToast(`Practice room discarded.`,`success`);return}if(e.room.hostId!==e.session.localPlayerId){this.handleRoomError({message:`Only the room host can discard the room.`});return}this.socket.emit(`discard_room`,{},e=>{e?.ok||this.handleRoomError({message:e?.error??`Unable to discard room.`})})}}toggleReady(){let e=this.store.getState(),t=e.room,n=e.session.localPlayerId,r=t?.players.find(e=>e.id===n);if(!(!t||!r)){if(t.source===`offline`){let e=r.status===`ready`?`unready`:`ready`,i=t.players.map(t=>t.id===n?{...t,status:e,connectionState:{...t.connectionState,note:e===`ready`?`Ready for practice toss`:`Reviewing practice settings`,lastSyncAt:Date.now()}}:t);this.store.dispatch({type:G.SET_ROOM,payload:{...t,players:i}}),this.store.dispatch({type:G.PATCH_UI,payload:{connectionBanner:e===`ready`?`Practice lobby ready. Start the toss when you want.`:`Practice lobby unlocked. Change settings or lineups before readying again.`}});return}this.socket.emit(`player_ready`,{ready:r.status!==`ready`},e=>{e?.ok||this.handleRoomError({message:e?.error??`Unable to change ready state.`})})}}removePlayer(e){let t=this.store.getState();if(!(!t.room||t.room.hostId!==t.session.localPlayerId||!e||e===t.room.hostId)){if(t.room.source===`offline`){this.pushToast(`AI players stay fixed in practice mode.`,`info`);return}this.socket.emit(`kick_player`,{playerId:e},n=>{if(n?.ok){let n=t.room?.players.find(t=>t.id===e)?.name??`Player`;this.pushToast(`${n} was removed from the room.`,`success`);return}this.handleRoomError({message:n?.error??`Unable to remove player.`})})}}},Yn=document.querySelector(`#app`);if(!(Yn instanceof HTMLElement))throw Error(`App root not found.`);var Xn=St(xt(),Le),Zn=new jn,Qn=new yt({root:Yn,store:Xn,roomService:new Jn({store:Xn,socket:Zn}),matchService:new Cn({store:Xn,socket:Zn}),signalSystem:new ot({store:Xn,socket:Zn}),socket:Zn});bt(Xn,_t),Qn.mount();